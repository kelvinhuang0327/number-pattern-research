...
Computes for the most recent 10 draws per lottery type:
  ...
  - with_prediction_row (strategies with >= 1 replay row for this draw)
  - coverage_pct
...
DEFAULT_DB_PATH = PROJECT_ROOT / "lottery_api" / "data" / "lottery_v2.db"
...
def run_coverage_matrix(db_path: Path | str | None = None) -> dict[str, Any]:
    ...
    if not target_db.exists():
        result["error"] = f"Database path not found: {target_db}"
        return result
    ...
    try:
        for lt in LOTTERY_TYPES:
            ...
            for row in draws:
                ...
                coverage_pct = (
                    (with_prediction / total_strategies * 100)
                ...
                for sr in strategy_rows:
                    for tl in (sr["truth_levels"] or "UNKNOWN").split(","):
                        truth_level_counts[tl] = (
                            truth_level_counts.get(tl, 0) + 1
                        )

                # Lifecycle breakdown (strategies with rows vs without)
                strategies_with_rows = {
                    sr["strategy_id"] for sr in strategy_rows
                }
                strategies_without = [
                    s for s in canonical_strategies
                    if s not in strategies_with_rows
                ]
                ...
                draws_data.append({
                    ...
                    "without_prediction_row": (
                        total_strategies - with_prediction
                    ),
                    "coverage_pct": round(coverage_pct, 1),
                    "by_truth_level": truth_level_counts,
                    "strategies_without_row": strategies_without,
                    "total_replay_rows": sum(
                        sr["row_count"] for sr in strategy_rows
                    ),
                })
            ...
            result["by_lottery"][lt] = {
                ...
                "summary": {
                    "avg_coverage_pct": round(
                        sum(d["coverage_pct"] for d in draws_data)
                        / len(draws_data),
                        1,
                    ) if draws_data else 0.0,
                    "draws_with_any_row": sum(
                        1 for d in draws_data
                        if d["with_prediction_row"] > 0
                    ),
                    "draws_with_full_coverage": sum(
                        1 for d in draws_data
                        if (
                            d["with_prediction_row"] >= total_strategies
...
def main():
    ...
    target_db = args.db_path if args.db_path else DEFAULT_DB_PATH
    ...
    matrix = run_coverage_matrix(db_path=target_db)
    ...
    for lt, data in matrix.get("by_lottery", {}).items():
        count = data["canonical_strategy_count"]
        print(f"\n{lt} (canonical strategies: {count})")
        for d in data["draws"]:
            print(
                f"  {d['draw_id']} ({d['draw_date']}): "
                f"{d['with_prediction_row']}/"
                f"{d['total_strategies_in_registry']} = "
    ...
    if args.json_out:
        ...
        default_out = (
            OUTPUT_DIR / f"p0_per_draw_coverage_matrix_{log_date}.json"
        )
