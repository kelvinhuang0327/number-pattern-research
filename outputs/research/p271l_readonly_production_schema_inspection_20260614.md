...
`lottery_api/data/lottery_v2.db` was opened **exactly once**, **immutable + read-only**
...
- **Actual prospective state: `ABSENT_CLEAN`** — the production schema currently contains
  **zero** `prospective_*` objects. A future controlled apply would therefore be a
  clean **additive** install (no partial/incompatible/stray prospective schema, no
  legacy collision).
- **Deployed schema:** 17 tables, 42 indexes, 1 view(s),
  0 triggers (61 objects total).
...
- Prior task classification: **`P271L_WRITER_QUIESCENCE_VERIFIED`** (effective start
  ...
  for this inspection; restoration requires a separate authorization.
...
- **SQLite authorizer** installed before any inspection query; allows only
  `SELECT`/`READ`/approved functions and a fixed allowlist of read-only introspection
...
**Actual-schema finding (beyond source-only preflight):** the deployed schema also
contains tables not documented in `database.py`'s legacy set —
`agent_locks`, `agent_task_runs`, `agent_tasks`, `prediction_explanations`, `prediction_review_status`, `review_actions`, `review_findings`, `review_hypotheses`, `review_sessions`, `shadow_experiments`, `snapshot_schedule` —
created by other source modules (orchestrator agent tables, prediction explanations).
None are `prospective_`-prefixed, so they do not affect the apply collision result.
...
The production database contains **no** prospective schema of any kind — neither the
full contract, nor a partial subset, nor an incompatible version, nor stray
`prospective_*` objects.

## 11. Collision / compatibility result

`collision_free_vs_legacy = True`;
contract names already present in deployed = none;
legacy name collisions = none. A future
CREATE-IF-NOT-EXISTS additive apply of the P271J contract is collision-free against the
...
- Governance: **HOLD / WAITING_FOR_USER_AUTHORIZATION** · Services remain stopped: **True**
  (restoration requires separate authorization).
