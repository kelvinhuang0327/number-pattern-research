#!/usr/bin/env python3
"""
P19 — Power Lotto Single Strategy Replay Dry-Run (1500 draws, read-only).

Selects one ONLINE POWER_LOTTO strategy (fourier_rhythm_3bet), generates
causal predictions for the most recent 1500 draws using only past history,
compares against real draw results, and writes a page-ready JSON report.
...
- prediction_cutoff_date derived from history_cutoff_draw JOIN draws.date.
- prediction_generated_at = UTC run timestamp.
...
_OUT_FILE = _OUT_DIR / "p19_powerlotto_single_strategy_replay_dry_run_20260520.json"
...
_PREFERRED = ["fourier_rhythm_3bet", "power_precision_3bet", "power_orthogonal_5bet"]
...
def _provenance_hash(strategy_id: str, draw: str, predicted: list[int]) -> str:
    raw = f"{strategy_id}:{draw}:{sorted(predicted)}"
...
def _select_strategy() -> dict | None:
    ...
    if not candidates:
        return None
    candidates.sort(
        key=lambda s: _PREFERRED.index(s["strategy_id"]) if s["strategy_id"] in _PREFERRED else len(_PREFERRED)
    )
    return candidates[0]
...
def run(db_path: Path = _DB_PATH, out_file: Path = _OUT_FILE) -> dict:
    ...
    if meta is None:
        result = _blocked(rows_before, "P19_BLOCKED_BY_NO_EXECUTABLE_POWER_LOTTO_STRATEGY",
                          "No ONLINE POWER_LOTTO adapter in registry")
    ...
    total     = len(all_draws)
    if total < 2:
        result = _blocked(rows_before, "P19_BLOCKED_BY_INSUFFICIENT_POWER_LOTTO_DRAWS",
    ...
    for draw_row in window_draws:
        ...
        try:
            ...
            _add(candidates, by_status, block_reasons, {
                ...
                "source_trace": f"parse_error:{exc}", "provenance_hash": None,
        ...
        if len(history) < adapter.meta.min_history:
            _add(candidates, by_status, block_reasons, {
                ...
                "hit_numbers": None, "hit_count": None, "special_hit": None,
                "prediction_cutoff_date": None, "prediction_generated_at": run_ts,
                "source_trace": f"need={adapter.meta.min_history},have={len(history)}", "provenance_hash": None,
                "truth_level": TRUTH_LEVEL, "dry_run_only": True,
                "would_insert": False, "counts_as_success": False,
            })
            continue
        ...
        # prediction_cutoff_date = date of the last history draw
    ...
    if rows_after != rows_before:
        classification = "P19_BLOCKED_BY_PRODUCTION_DB_DRIFT"
    elif n_ready == 0:
        classification = "P19_BLOCKED_BY_NO_EXECUTABLE_POWER_LOTTO_STRATEGY"
    elif available < TARGET_WINDOW:
        classification = "P19_POWERLOTTO_PARTIAL_WINDOW_READY"
    else:
...
def _write(data, path):
    ...
    print(f"[P19] written → {path}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="P19 POWER_LOTTO single-strategy replay dry-run")
    ...
    print(f"[P19] classification = {result['final_classification']}")
    print(f"[P19] generated={result['generated_candidates']} ready={result['ready_candidates']} blocked={result['blocked_candidates']}")
    print(f"[P19] rows_before={result['production_rows_before']} rows_after={result['production_rows_after']}")
