"""Application Use Case: Single Bet Generation for Daily 539."""
from typing import List, Dict, Any
import logging

from lottery.prediction.daily539.models import (
...
class Daily539GenerateOneBetUseCase:
    """
    Canonical use case for Daily 539 single bet generation.
    Enforces strict product contract:
      ...
      - Enforces domain rules: 5 unique numbers, range [1..39], no special number for DAILY_539
      - Fails closed on unknown strategy, non-ONLINE status, adapter mismatch, or invalid output
    """

    def execute(self, request: Daily539GenerateBetRequest) -> Daily539GenerateBetResult:
        if not isinstance(request, Daily539GenerateBetRequest):
            raise InvalidOutputError("Request must be an instance of Daily539GenerateBetRequest")
        ...
        lottery_type = request.lottery_type or "DAILY_539"
        if lottery_type != "DAILY_539":
            raise InvalidStrategyError(f"Daily539 vertical supports DAILY_539 only, got {lottery_type}")
        ...
        self._validate_daily539_numbers(numbers, special, request.strategy_id)

        return Daily539GenerateBetResult(
    ...
    def _validate_daily539_numbers(self, numbers: Any, special: Any, strategy_id: str) -> None:
        if not isinstance(numbers, list):
            raise InvalidOutputError(f"{strategy_id}: numbers must be a list")
        if len(numbers) != 5:
            raise InvalidOutputError(f"{strategy_id}: Daily 539 requires exactly 5 numbers, got {len(numbers)}")
        ...
        if not all(1 <= n <= 39 for n in numbers):
            raise InvalidOutputError(f"{strategy_id}: numbers out of range [1..39]: {numbers}")
        if len(set(numbers)) != 5:
            raise InvalidOutputError(f"{strategy_id}: numbers contain duplicates: {numbers}")
        if special is not None:
            raise InvalidOutputError(f"{strategy_id}: Daily 539 must not have special number, got {special}")
