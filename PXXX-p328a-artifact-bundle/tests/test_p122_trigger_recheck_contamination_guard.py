"""
Tests for P122: Trigger Recheck / Cross-Project Contamination Guard
"""
...
JSON_ARTIFACT = PROJECT_ROOT / "outputs" / "replay" / "p122_trigger_recheck_contamination_guard_20260527.json"
MD_ARTIFACT = PROJECT_ROOT / "docs" / "replay" / "p122_trigger_recheck_contamination_guard_20260527.md"
SCRIPT = PROJECT_ROOT / "scripts" / "p122_trigger_recheck_contamination_guard.py"
...
VALID_CLASSIFICATIONS = {
    ...
    "P122_BLOCKED_BY_PREFLIGHT",
    "P122_BLOCKED_BY_DB_DRIFT",
    "P122_BLOCKED_BY_GUARD_FAILURE",
    "P122_BLOCKED_BY_TEST_FAILURE",
    "P122_BLOCKED_BY_FORBIDDEN_FILES_STAGED",
    "P122_BLOCKED_BY_SCOPE_VIOLATION",
...
def test_task_id(artifact):
    assert artifact["task_id"] == "P122_TRIGGER_RECHECK_CONTAMINATION_GUARD"
...
def test_p121_reference_exists(artifact):
    assert "p121_reference" in artifact
    assert "classification" in artifact["p121_reference"]
    assert "artifact_path" in artifact["p121_reference"]
...
def test_all_blocked_classification(artifact):
    ...
    contamination = artifact["cross_project_contamination_guard"]["contamination_detected"]
    if not any([p108, p117p, p117f, p118, p4star]) and not contamination:
        assert artifact["classification"] == "P122_ALL_TRIGGERS_STILL_BLOCKED"
...
def test_md_contains_contamination_guard(md_text):
    assert "contamination" in md_text.lower() or "Contamination" in md_text
