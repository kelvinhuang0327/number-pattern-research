#!/usr/bin/env python3
"""
P42 Wave 3 — BIG_LOTTO Dry-Run + Temp DB Rehearsal
====================================================
P42: Wires replay adapter wrappers for the 6 Wave 3 BIG_LOTTO strategies,
...
AUTHORISATION GATE: This script is read-only with respect to the production DB.
  ...
  - Writes: /tmp/p42_temp_rehearsal.db (temp DB, created fresh each run)
  - Output: outputs/replay/p42_wave3_biglotto_dryrun_rehearsal_20260524.json
            outputs/replay/p42_temp_rehearsal_20260524.json

HARD RULES:
  - Production DB MUST remain at exactly 28960 rows after this script.
  - Temp DB rows are NEVER copied to production DB in P42.
  - lifecycle for all dry-run rows MUST be "DRY_RUN" (NOT "ONLINE").
  - Adapter wrappers are NOT registered in replay_strategy_registry.
  - Special number policy: NOT_PREDICTED_WAVE3 (predicted_special=None, special_hit=0)

Wave 3 Strategies (6 total × 1500 draws = 9000 rows):
...
P43 production apply requires separate explicit authorization.

Usage:
  cd /path/to/LotteryNew
  .venv/bin/python scripts/p42_wave3_biglotto_dryrun_rehearsal.py
"""
...
TEMP_DB_PATH = Path("/tmp/p42_temp_rehearsal.db")
OUTPUT_DIR = REPO_ROOT / "outputs" / "replay"
DRYRUN_OUTPUT = OUTPUT_DIR / "p42_wave3_biglotto_dryrun_rehearsal_20260524.json"
REHEARSAL_OUTPUT = OUTPUT_DIR / "p42_temp_rehearsal_20260524.json"

EXPECTED_PROD_ROWS = 28960
WINDOW_PERIODS = 1500     # dry-run spans last 1500 BIG_LOTTO draws
STRATEGIES_COUNT = 6
EXPECTED_DRY_RUN_ROWS = WINDOW_PERIODS * STRATEGIES_COUNT  # 9000

TRUTH_LEVEL = "P42_WAVE3_BIGLOTTO_DRY_RUN"
RUN_ID = "p42_wave3_biglotto_dryrun_20260524"
...
# DAILY_539 / POWER_LOTTO strategy IDs must NOT appear in Wave 3 BIG_LOTTO
FORBIDDEN_STRATEGY_PREFIXES = ("539", "power", "daily", "acb_", "markov_1bet_539",
                                "midfreq", "fourier_rhythm", "p0b_", "p0c_",
                                "zone_gap", "orthogonal")
...
def _assert_prod_rows_unchanged(expected: int = EXPECTED_PROD_ROWS) -> int:
    ...
    if count != expected:
        raise RuntimeError(
            ...
            "Aborting P42 — production DB must not be modified."
        )
    return count
...
def _load_biglotto_draws() -> List[dict]:
    """
    Load all BIG_LOTTO draws from production DB, sorted chronologically (draw# ASC).
...
def _to_db_row(row: dict, now_str: str) -> dict:
    ...
    return {
        ...
        "source": "P42_WAVE3_BIGLOTTO_DRYRUN",
...
def _validate_rows(rows: List[dict]) -> dict:
    """Validate schema constraints for all dry-run BIG_LOTTO rows."""
    errors = []
    for i, row in enumerate(rows):
        ...
        if lottery != "BIG_LOTTO":
            errors.append(f"Row {i} ({sid}): lottery_type={lottery} expected BIG_LOTTO")
        if lifecycle == "ONLINE":
        ...
        if pred is not None:
            ...
            elif not all(1 <= n <= 49 for n in pred):
        ...
        if pred_special is not None:
            errors.append(f"Row {i} ({sid}): predicted_special should be None (Wave 3 policy), got {pred_special}")
        if special_hit != 0:
            errors.append(f"Row {i} ({sid}): special_hit should be 0 (Wave 3 policy), got {special_hit}")
...
def _check_no_future_leakage(rows: List[dict]) -> dict:
    """
    Verify prediction_cutoff_date < draw_date for all rows with PREDICTED status.
    ...
    return {
        ...
        "violations": violations[:5],  # show first 5 if any
...
def main() -> dict:
    ...
    now_str = started_at
    logger.info("=== P42 Wave 3 BIG_LOTTO Dry-Run + Temp Rehearsal ===")
    ...
    if total_draws < WINDOW_PERIODS + 100:
        raise RuntimeError(
            f"Need at least {WINDOW_PERIODS + 100} BIG_LOTTO draws, got {total_draws}"
        )

    # ── Import Wave 3 adapters ────────────────────────────────────────────────
    from lottery_api.models.p42_wave3_biglotto_adapters import WAVE3_ADAPTERS, generate_dryrun_rows
    logger.info("Wave 3 adapters loaded: %d", len(WAVE3_ADAPTERS))
    ...
    format_ok = all(
        ...
        and all(1 <= n <= 49 for n in r["predicted_numbers"])
    ...
    # ── Rehearsal JSON ────────────────────────────────────────────────────────
    rehearsal = {
        "phase": "P42_WAVE3_BIGLOTTO_DRY_RUN",
        "run_id": RUN_ID,
    ...
    dryrun = {
        ...
        "leakage_check": leakage_check,
        "all_rehearsals_pass": all_rehearsals_pass,
        "classification": "P42_WAVE3_BIGLOTTO_DRYRUN_REHEARSAL_READY" if all_rehearsals_pass
                          else "P42_WAVE3_BIGLOTTO_DRYRUN_REHEARSAL_FAIL",
        "lifecycle_semantics": {
            "all_rows_lifecycle": "DRY_RUN",
            "online_rows": 0,
            "retired_rows": 0,
            "note": "P42 is dry-run only. Production apply requires P43 authorization.",
        },
        "production_apply_readiness": {
            "ready": all_rehearsals_pass,
            "requires_p43_authorization": True,
    ...
    logger.info("BIG_LOTTO format ok:  %s", format_ok)
