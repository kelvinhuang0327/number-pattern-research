"""Focused static tests for the P537D client-side filter/export controls added
on top of the P537B shortlist-robustness-review minimal UI. No DB, no route
...
def test_filter_bar_controls_present():
    section = _section()
    assert 'id="p537d-filter-bar"' in section
    assert 'id="p537d-filter-category"' in section
    assert 'id="p537d-filter-lottery"' in section
    assert 'id="p537d-filter-search"' in section
    assert 'id="p537d-filter-reset"' in section
    assert 'id="p537d-export-csv"' in section


def test_category_filter_options_match_five_candidate_sections():
    section = _section()
    filter_block = section.split('id="p537d-filter-category"', 1)[1].split("</select>", 1)[0]
    for value in ("ALL", "stable", "short_window_spike", "combination", "cross_lottery", "insufficient"):
        assert f'value="{value}"' in filter_block


def test_lottery_filter_options_match_known_lottery_types():
    section = _section()
    filter_block = section.split('id="p537d-filter-lottery"', 1)[1].split("</select>", 1)[0]
...
def test_candidate_cards_have_stable_ids_for_category_visibility_toggle():
    section = _section()
    assert 'id="p537b-card-stable"' in section
    assert 'id="p537b-card-spike"' in section
    assert 'id="p537b-card-combo"' in section
    assert 'id="p537b-card-cross"' in section
    assert 'id="p537b-card-insufficient"' in section


def test_reset_button_wired_to_reset_function():
    script = _script()
    assert "resetFiltersP537D" in script
    assert "addEventListener('click', resetFiltersP537D)" in script


def test_export_button_wired_to_export_function():
    script = _script()
    assert "exportVisibleCsvP537D" in script
    assert "addEventListener('click', exportVisibleCsvP537D)" in script


def test_filters_only_operate_on_already_loaded_data_no_new_fetch():
    script = _script()
    # still only the one fetch call from P537B itself
    assert script.count("fetch(") == 1
    assert "fetch(base + '/api/replay/shortlist-robustness-review')" in script
    assert "_p537bData = data" in script


def test_csv_export_builds_from_filtered_in_memory_rows_not_dom():
    script = _script()
    assert "buildVisibleCsvP537D" in script
    assert "getFilteredStableP537D" in script
    assert "getFilteredSpikeP537D" in script
    assert "getFilteredComboP537D" in script
    assert "getFilteredCrossP537D" in script
    assert "getFilteredInsufficientP537D" in script
...
def test_category_filter_narrows_to_single_section_and_all_shows_everything():
    script = _script()
    assert "f.category !== 'ALL' && f.category !== 'stable'" in script
    ...
    assert "f.category !== 'ALL' && f.category !== 'insufficient'" in script
