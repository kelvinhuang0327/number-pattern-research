...
DEFAULT_SETTINGS = {
    ...
    "cto_planner_model": "",
    # LLM Planner multi-provider settings (safe defaults — no auto LLM calls on fresh install)
    "planner_llm_provider": "local",
    "planner_llm_model": "auto",
    "planner_llm_enabled": "0",
    "planner_llm_mode": "off",
    "planner_budget_guard_enabled": "1",
    "planner_daily_call_cap": "0",
    "planner_timeout_seconds": "300",
    "planner_max_output_chars": "12000",
    # CTO Review Provider settings (safe defaults — local review only, no LLM calls)
    "cto_review_provider": "local",
    "cto_review_model": "auto",
    "cto_review_mode": "local_review",
    "cto_review_daily_call_cap": "0",
    "cto_review_timeout_seconds": "300",
    "cto_review_max_context_chars": "12000",
    "cto_review_max_output_chars": "6000",
}
...
def init_db():
    ...
    try:
        ...
        # ── LLM Attribution columns on agent_tasks ────────────────────────────
        _existing_at_attr_cols = {r["name"] for r in c.execute("PRAGMA table_info(agent_tasks)").fetchall()}
        for _col, _typ in [
            ("planner_agent_label", "TEXT"),
            ("worker_agent_label", "TEXT"),
            ("llm_call_count", "INTEGER DEFAULT 0"),
        ]:
            if _col not in _existing_at_attr_cols:
                c.execute(f"ALTER TABLE agent_tasks ADD COLUMN {_col} {_typ}")
        ...
        """)
        c.execute("CREATE INDEX IF NOT EXISTS idx_lue_day      ON llm_usage_events(day_label)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_lue_task_id  ON llm_usage_events(task_id)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_lue_agent    ON llm_usage_events(agent_label)")

        # Migration-safe: add new columns when not present
        _lue_cols = {row[1] for row in c.execute("PRAGMA table_info(llm_usage_events)").fetchall()}
        _lue_new_cols = [
            ("usage_role",          "TEXT"),
            ("run_id",              "TEXT"),
            ...
            ("rate_limit_reset_raw","TEXT"),
            ("rate_limit_reset_at", "TEXT"),
            ("raw_usage_text",      "TEXT"),
            ("parsed",              "INTEGER DEFAULT 0"),
            ("parse_source",        "TEXT"),
        ]
        for _col_name, _col_def in _lue_new_cols:
            if _col_name not in _lue_cols:
                c.execute(f"ALTER TABLE llm_usage_events ADD COLUMN {_col_name} {_col_def}")
        ...
        c.execute("""
            CREATE TABLE IF NOT EXISTS llm_audit_events (
                ...
                rate_limit_used_pct REAL,
                rate_limit_reset_raw TEXT,
                raw_usage_excerpt   TEXT,
                would_have_called   TEXT,
                day_label           TEXT,
                created_at          TEXT    NOT NULL
            )
        """)
        c.execute("CREATE INDEX IF NOT EXISTS idx_lae_corr   ON llm_audit_events(correlation_id)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_lae_day    ON llm_audit_events(day_label)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_lae_task   ON llm_audit_events(task_id)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_lae_type   ON llm_audit_events(event_type)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_lae_prov   ON llm_audit_events(provider)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_lae_runner ON llm_audit_events(runner_type)")
...
# ── LLM Usage Role Mapping ────────────────────────────────────────────────────

_RUNNER_TYPE_TO_ROLE: dict = {
    "planner": "planner",
    "worker": "worker",
    "light_worker": "worker",
...
def _runner_type_to_role(runner_type: str) -> str:
    """Map runner_type to a usage_role string. Returns 'unknown' for unmapped types."""
    return _RUNNER_TYPE_TO_ROLE.get(str(runner_type or ""), "unknown")
...
def log_llm_usage_event(
    ...
    **metadata,
    ...
    """Write one row to llm_usage_events. Returns the new row id.

    If usage_text is provided, parse_llm_usage_text() is called and the
    resulting metrics are stored alongside the event.  Parser exceptions are
    silently caught so they never break task finalisation.
    """
    import json as _json
    from orchestrator import common as _common
    now = datetime.now(timezone.utc).isoformat()
    dl = day_label or _common.local_day_label()
    # Resolve usage_role from runner_type if not explicitly provided
    resolved_role = usage_role or _runner_type_to_role(runner_type)
    meta_str = _json.dumps(metadata, ensure_ascii=False) if metadata else None

    # ── Optional: parse usage_text for token/premium metrics ────────────────
    metrics: dict = {}
    if usage_text:
        try:
            from orchestrator.llm_usage_parser import parse_llm_usage_text
            metrics = parse_llm_usage_text(
                usage_text,
                parse_source=parse_source or runner_type or "unknown",
            )
        except Exception as _exc:  # pylint: disable=broad-except
            logger.warning("[db] usage_text parse error: %s", _exc)
            metrics = {"parsed": False, "raw_usage_text": (usage_text or "")[:500]}

    conn = get_conn()
    try:
        c = conn.execute(
            """INSERT INTO llm_usage_events
               (task_id, runner_type, usage_role, run_id, provider, model, agent_label,
                call_count, event_type, day_label, created_at, metadata_json,
                premium_requests, input_tokens, output_tokens, cached_tokens,
                total_tokens, rate_limit_type, rate_limit_used_pct,
                rate_limit_reset_raw, rate_limit_reset_at,
                raw_usage_text, parsed, parse_source)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
                       ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (task_id, runner_type, resolved_role, run_id,
             str(provider or ""), str(model or ""),
             str(agent_label or ""), call_count, event_type, dl, now, meta_str,
             metrics.get("premium_requests"),
             metrics.get("input_tokens"),
             metrics.get("output_tokens"),
             metrics.get("cached_tokens"),
             metrics.get("total_tokens"),
             metrics.get("rate_limit_type"),
             metrics.get("rate_limit_used_pct"),
             metrics.get("rate_limit_reset_raw"),
             metrics.get("rate_limit_reset_at"),
             metrics.get("raw_usage_text"),
             1 if metrics.get("parsed") else 0,
             metrics.get("parse_source")),
        )
        conn.commit()
        return c.lastrowid
    finally:
...
def update_llm_usage_event_metrics(event_id: int, metrics: dict) -> None:
    """Back-fill token/premium/rate-limit columns on an existing llm_usage_events row.

    Only updates columns whose value is not None in the metrics dict.
    Safe to call multiple times; subsequent calls overwrite earlier values.
    Silently does nothing if event_id does not exist.
    """
    if not metrics:
        return
    _col_map = {
    ...
    try:
        conn.execute(
            f"UPDATE llm_usage_events SET {', '.join(sets)} WHERE id = ?",
            vals,
        )
        conn.commit()
    finally:
...
def find_latest_llm_usage_event_for_task(
    task_id: int,
    usage_role: Optional[str] = None,
    runner_type: Optional[str] = None,
) -> Optional[dict]:
    """Return the most recent llm_usage_events row for *task_id*.

    Optionally filtered by usage_role and/or runner_type.
    Returns None when no matching row exists.
    """
    conditions = ["task_id = ?"]
    params: list = [task_id]
    if usage_role:
        # Also match rows where usage_role IS NULL — historical events written before
        # the usage_role column was populated so they still get backfilled.
        conditions.append("(usage_role = ? OR usage_role IS NULL)")
        params.append(usage_role)
    if runner_type:
        conditions.append("runner_type = ?")
        params.append(runner_type)
    where = " AND ".join(conditions)
    conn = get_conn()
    try:
        row = conn.execute(
            f"SELECT * FROM llm_usage_events WHERE {where} ORDER BY id DESC LIMIT 1",
            params,
        ).fetchone()
...
def backfill_llm_usage_metrics_for_task(
    task_id: int,
    output_text: str,
    parse_source: str,
    *,
    ...
    """Parse *output_text* and back-fill metrics onto the most recent event for task_id.

    Behaviour:
    - output_text empty/whitespace → no-op (not an error)
    - parser error → log warning, never raises
    - existing event already parsed=1 with same parse_source → skip (dedupe)
    - existing event parsed=0 → update metrics
    - no existing event + parsed metrics found → insert new minimal event
    - no existing event + no usage found → no-op
    """
    if not output_text or not output_text.strip():
        return
    try:
        from orchestrator.llm_usage_parser import parse_llm_usage_text
        metrics = parse_llm_usage_text(output_text, parse_source=parse_source)
    except Exception as _exc:  # pylint: disable=broad-except
        logger.warning("[db] backfill_llm_usage_metrics_for_task parse error: %s", _exc)
        return

    event = find_latest_llm_usage_event_for_task(
        task_id, usage_role=usage_role, runner_type=runner_type
    )

    if event is not None:
        # Dedupe: same parse_source already parsed → skip
        if event.get("parsed") == 1 and event.get("parse_source") == parse_source:
            logger.debug(
                "[db] backfill skipped: event %d already parsed (source=%s)",
                event["id"], parse_source,
            )
            return
        update_llm_usage_event_metrics(event["id"], metrics)
        logger.debug(
            "[db] backfill: updated event %d for task %d (source=%s)",
            event["id"], task_id, parse_source,
        )
    else:
        # No existing event — only insert when usage was actually found
        if not metrics.get("parsed"):
            return
        try:
            from orchestrator import common as _common
            now = datetime.now(timezone.utc).isoformat()
            dl = _common.local_day_label()
            conn = get_conn()
            try:
                c = conn.execute(
                    """INSERT INTO llm_usage_events
                       (task_id, runner_type, usage_role, provider, model, agent_label,
                        call_count, event_type, day_label, created_at,
                        premium_requests, input_tokens, output_tokens, cached_tokens,
                        total_tokens, rate_limit_type, rate_limit_used_pct,
                        rate_limit_reset_raw, rate_limit_reset_at,
                        raw_usage_text, parsed, parse_source)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
                               ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                    (task_id, runner_type or "worker", usage_role or "worker",
                     "", "", "worker_backfill",
                     1, "worker_llm_usage_parse", dl, now,
                     metrics.get("premium_requests"),
                     metrics.get("input_tokens"),
                     metrics.get("output_tokens"),
                     metrics.get("cached_tokens"),
                ...
                conn.commit()
                logger.debug(
                    "[db] backfill: inserted new event id=%d for task %d (source=%s)",
                    c.lastrowid, task_id, parse_source,
                )
            finally:
                conn.close()
        except Exception as _exc:  # pylint: disable=broad-except
            logger.warning("[db] backfill_insert error: %s", _exc)


def get_today_llm_usage_by_agent(day_label: str = None) -> list:
    """Aggregate LLM call counts by agent_label for today (Asia/Taipei).

    Primary grouping is agent_label (the display label shown in UI).
    runner_type breakdown is nested as a detail list for debugging.
    """
    from orchestrator import common as _common
    dl = day_label or _common.local_day_label()
    conn = get_conn()
    try:
        # Top-level: aggregate by agent_label only
        rows = conn.execute(
            """SELECT agent_label,
        ...
        # Attach runner_type breakdown for debugging
        detail_rows = conn.execute(
            """SELECT agent_label, runner_type,
                      SUM(call_count) AS calls
               FROM llm_usage_events
               WHERE day_label = ?
               GROUP BY agent_label, runner_type""",
            (dl,),
        ).fetchall()
        detail_map: dict = {}
        for d in detail_rows:
            detail_map.setdefault(d["agent_label"], []).append(
                {"runner_type": d["runner_type"], "calls": d["calls"]}
            )
        for r in results:
            r["by_runner"] = detail_map.get(r["agent_label"], [])
...
def get_today_llm_usage_by_role(day_label: str = None) -> dict:
    """Return today's LLM call counts grouped by role (planner/worker/cto/unknown).

    Each role contains a list of agents with their call counts.
    Usage_role is read from the DB column; for old rows without it, falls back
    to mapping via runner_type.
    Excludes rows with provider IN ('local','adaptive_regime','') per attribution rules.
    """
    from orchestrator import common as _common
    dl = day_label or _common.local_day_label()
    conn = get_conn()
    try:
        # Per-role, per-agent aggregation (includes token metrics)
        rows = conn.execute(
            """SELECT
                 COALESCE(
                   NULLIF(usage_role, ''),
                   CASE runner_type
                     ...
                     WHEN 'light_worker'  THEN 'worker'
                     WHEN 'copilot-daemon' THEN 'worker'
                     WHEN 'cto'           THEN 'cto'
                     WHEN 'cto_review'    THEN 'cto'
                     WHEN 'cto_planner'   THEN 'cto'
                     WHEN 'cto_worker'    THEN 'cto'
                     ELSE 'unknown'
                   END
                 ) AS role,
                 agent_label,
                 ...
                 SUM(call_count)          AS total_calls,
                 runner_type,
                 SUM(COALESCE(premium_requests, 0)) AS premium_requests,
                 SUM(COALESCE(input_tokens, 0))     AS input_tokens,
                 SUM(COALESCE(output_tokens, 0))    AS output_tokens,
                 SUM(COALESCE(cached_tokens, 0))    AS cached_tokens,
                 SUM(COALESCE(total_tokens, 0))     AS total_tokens,
                 MAX(rate_limit_used_pct)            AS rate_limit_used_pct,
                 MAX(rate_limit_type)                AS rate_limit_type,
                 MAX(rate_limit_reset_at)            AS rate_limit_reset_at
               FROM llm_usage_events
               WHERE day_label = ?
               GROUP BY role, agent_label, runner_type
               ORDER BY role, total_calls DESC""",
            (dl,),
        ).fetchall()

        # Build role-grouped structure
        _ROLE_ORDER = ["planner", "worker", "cto", "unknown"]
        role_map: dict = {r: {} for r in _ROLE_ORDER}

        for row in rows:
            role = row["role"] or "unknown"
            if role not in role_map:
                role_map[role] = {}
            al = row["agent_label"] or "Unknown"
            if al not in role_map[role]:
                role_map[role][al] = {
                    "agent_label": al,
            ...
            ag = role_map[role][al]
            ag["total_calls"] += row["total_calls"]
            ag["premium_requests"] = round(ag["premium_requests"] + (row["premium_requests"] or 0), 6)
            ag["input_tokens"]  += int(row["input_tokens"]  or 0)
            ag["output_tokens"] += int(row["output_tokens"] or 0)
            ag["cached_tokens"] += int(row["cached_tokens"] or 0)
            ag["total_tokens"]  += int(row["total_tokens"]  or 0)
            ag["by_runner"].append({
                "runner_type": row["runner_type"] or "",
                "calls": row["total_calls"],
            })
            # Propagate latest rate-limit warning per agent
            if row["rate_limit_used_pct"] is not None:
                existing = ag.get("latest_rate_limit_warning")
                if existing is None or (row["rate_limit_used_pct"] or 0) > (existing.get("used_pct") or 0):
                    ag["latest_rate_limit_warning"] = {
                        "type": row["rate_limit_type"],
                        "used_pct": row["rate_limit_used_pct"],
                        "reset_at": row["rate_limit_reset_at"],
                    }

        roles_out = []
        for role in _ROLE_ORDER:
            agents = list(role_map.get(role, {}).values())
            roles_out.append({
                "role": role,
                "label": role.capitalize() if role != "cto" else "CTO",
                "total_calls": sum(a["total_calls"] for a in agents),
                "premium_requests": round(sum(a["premium_requests"] for a in agents), 6),
                "input_tokens":  sum(a["input_tokens"]  for a in agents),
                "output_tokens": sum(a["output_tokens"] for a in agents),
                "cached_tokens": sum(a["cached_tokens"] for a in agents),
                "total_tokens":  sum(a["total_tokens"]  for a in agents),
                "items": agents,
            })

        total = sum(r["total_calls"] for r in roles_out)

        # Global token / premium totals for summary
        _sum_field = lambda field: sum(r[field] for r in roles_out)
        summary = {
            "total_calls": total,
            "planner_calls": role_map.get("planner") and sum(a["total_calls"] for a in role_map["planner"].values()) or 0,
            "worker_calls":  role_map.get("worker")  and sum(a["total_calls"] for a in role_map["worker"].values())  or 0,
            "cto_calls":     role_map.get("cto")     and sum(a["total_calls"] for a in role_map["cto"].values())     or 0,
            "unknown_calls": role_map.get("unknown") and sum(a["total_calls"] for a in role_map["unknown"].values()) or 0,
            # Token / premium aggregates
            "premium_requests_total": round(_sum_field("premium_requests"), 6),
            "input_tokens_total":  _sum_field("input_tokens"),
            "output_tokens_total": _sum_field("output_tokens"),
            "cached_tokens_total": _sum_field("cached_tokens"),
            "total_tokens":        _sum_field("total_tokens"),
            # Rate-limit warnings: collect all active ones
            "rate_limit_warnings": _collect_rate_limit_warnings(dl),
        }
...
def _collect_rate_limit_warnings(day_label: str) -> list:
    """Return distinct rate-limit warnings seen in today's events."""
    conn = get_conn()
    try:
        rows = conn.execute(
            """SELECT rate_limit_type, MAX(rate_limit_used_pct) AS used_pct,
                      MAX(rate_limit_reset_at) AS reset_at
               FROM llm_usage_events
               WHERE day_label = ? AND rate_limit_used_pct IS NOT NULL
               GROUP BY rate_limit_type
               ORDER BY used_pct DESC""",
            (day_label,),
        ).fetchall()
        return [
            {"type": r["rate_limit_type"], "used_pct": r["used_pct"], "reset_at": r["reset_at"]}
            for r in rows
        ]
    finally:
        conn.close()
...
def get_recent_llm_usage_events(limit: int = 20) -> list:
    """Return the most recent llm_usage_events rows with token/usage metrics.

    Suitable for the /llm-usage/recent API endpoint.
    Returns at most `limit` rows (capped at 200 for safety).
    """
    limit = min(int(limit), 200)
    conn = get_conn()
    try:
        rows = conn.execute(
            """SELECT id, task_id, run_id, runner_type, usage_role, provider, model,
                      agent_label, call_count, event_type, day_label, created_at,
                      premium_requests, input_tokens, output_tokens, cached_tokens,
                      total_tokens, rate_limit_type, rate_limit_used_pct,
                      rate_limit_reset_raw, rate_limit_reset_at,
                      raw_usage_text, parsed, parse_source
        ...
        for row in rows:
            r = dict(row)
            # Truncate raw_usage_text for the response
            if r.get("raw_usage_text"):
                r["raw_usage_excerpt"] = r["raw_usage_text"][:200]
            else:
                r["raw_usage_excerpt"] = None
            del r["raw_usage_text"]
            results.append(r)
        return results
    finally:
        conn.close()
...
def get_llm_usage_detail(day_label: str = None) -> dict:
    """Return comprehensive LLM usage detail for the dashboard.

    Primary source: llm_audit_events (has blocked/success/failed flags).
    Only counts LLM_CALL_RESULT and LLM_CALL_BLOCKED rows (not in-flight ATTEMPT rows).

    Returns a dict with:
        roles          - per-role summary (planner/worker/cto/unknown)
        by_provider    - breakdown by provider
        by_runner      - breakdown by runner_type + provider + agent_label
        by_agent       - breakdown by agent_label
        copilot_focus  - Copilot-Daemon specific aggregation
        top_tasks      - top 10 tasks by call count
        recent         - 10 most recent events with task title
        warnings       - overuse warning messages
    """
    from orchestrator import common as _common
    dl = day_label or _common.local_day_label()
    conn = get_conn()

    # ── Helper: map runner_type → role ────────────────────────────────────
    _ROLE_SQL = """COALESCE(NULLIF(usage_role,''),
        CASE runner_type
            WHEN 'planner'        THEN 'planner'
            WHEN 'worker'         THEN 'worker'
            WHEN 'light_worker'   THEN 'worker'
    ...
    _RELEVANT = "event_type IN ('LLM_CALL_RESULT', 'LLM_CALL_BLOCKED')"
    ...
    try:
        ...
        role_rows = conn.execute(
            f"""SELECT
                    ...
                    SUM(CASE WHEN success=0 AND blocked=0 THEN 1 ELSE 0 END) AS failed,
        ...
        for r in role_rows:
            ...
            role_map[role] = {
                ...
                "providers": [p for p in (r["providers"] or "").split(",") if p],
                "models": [m for m in (r["models"] or "").split(",") if m],
                "input_tokens": r["input_tokens"] or 0,
                "output_tokens": r["output_tokens"] or 0,
                "cached_tokens": r["cached_tokens"] or 0,
                "total_tokens": r["total_tokens"] or 0,
                "premium_requests": round(float(r["premium_requests"] or 0), 6),
                "rate_limit_type": r["rate_limit_type"],
                "rate_limit_used_pct": r["rate_limit_used_pct"],
            }
        roles_out = [
            role_map.get(r, {"role": r, "label": r.capitalize() if r != "cto" else "CTO",
                              "calls": 0, "successful": 0, "failed": 0, "blocked": 0,
                              "providers": [], "models": [], "input_tokens": 0,
                              "output_tokens": 0, "cached_tokens": 0, "total_tokens": 0,
                              "premium_requests": 0, "rate_limit_type": None,
                              "rate_limit_used_pct": None})
            for r in _ROLE_ORDER
        ] + [v for k, v in role_map.items() if k not in _ROLE_ORDER]
        ...
        by_provider = [dict(r) for r in prov_rows]

        # ── 3. By runner ───────────────────────────────────────────────────
        runner_rows = conn.execute(
            f"""SELECT
                    COALESCE(NULLIF(runner_type,''),'unknown')          AS runner_type,
                    COALESCE(NULLIF(provider,''),'unknown')             AS provider,
                    COALESCE(NULLIF(agent_label,''),'unknown')          AS agent_label,
                    COUNT(*)                                             AS calls,
                    SUM(CASE WHEN success=1 THEN 1 ELSE 0 END)          AS successful,
                    SUM(CASE WHEN success=0 AND blocked=0 THEN 1 ELSE 0 END) AS failed,
                    SUM(CASE WHEN blocked=1 THEN 1 ELSE 0 END)          AS blocked,
                    ...
                    MAX(CASE WHEN input_tokens>0 OR output_tokens>0 THEN 1 ELSE 0 END) AS has_tokens
                FROM llm_audit_events
                WHERE day_label=? AND {_RELEVANT}
                GROUP BY runner_type, provider, agent_label
                ORDER BY calls DESC""",
            (dl,),
        ).fetchall()
        by_runner = []
        for r in runner_rows:
            d = dict(r)
            d["token_status"] = "available" if d.pop("has_tokens", 0) else "unavailable"
            d["avg_duration_ms"] = round(d["avg_duration_ms"]) if d["avg_duration_ms"] is not None else None
            by_runner.append(d)
        ...
        cop_rows = conn.execute(
            f"""SELECT
                ...
                FROM llm_audit_events
                WHERE day_label=? AND {_RELEVANT}
                    AND (runner_type='copilot-daemon'
                         OR provider LIKE '%copilot%'
                         OR provider LIKE '%github-copilot%')
                GROUP BY runner_type, provider
                ORDER BY calls DESC""",
            (dl,),
        ).fetchall()
        cop_by_runner = [dict(r) for r in cop_rows]
        cop_total = sum(r["calls"] for r in cop_by_runner)
        cop_blocked = sum(r["blocked"] for r in cop_by_runner)
        cop_failed = sum(r["failed"] for r in cop_by_runner)
        cop_successful = sum(r["successful"] for r in cop_by_runner)

        cop_task_rows = conn.execute(
            f"""SELECT
                    e.task_id,
                    t.title        AS task_title,
                    t.status       AS task_status,
                    COUNT(*)       AS calls,
                    SUM(CASE WHEN e.blocked=1 THEN 1 ELSE 0 END) AS blocked,
                    SUM(CASE WHEN e.success=0 AND e.blocked=0 THEN 1 ELSE 0 END) AS failed,
                    MAX(e.created_at) AS last_call_at
                FROM llm_audit_events e
                LEFT JOIN agent_tasks t ON t.id = e.task_id
                WHERE e.day_label=? AND {_RELEVANT}
                    AND (e.runner_type='copilot-daemon'
                         OR e.provider LIKE '%copilot%')
        ...
        cop_top_tasks = [dict(r) for r in cop_task_rows]

        cop_recent = conn.execute(
            f"""SELECT
                    e.id, e.created_at, e.event_type, e.runner_type, e.provider,
                    e.agent_label, e.task_id, t.title AS task_title,
                    e.success, e.blocked, e.error, e.duration_ms,
        ...
        cop_recent_list = [dict(r) for r in cop_recent]

        # Token detail available only when input_tokens/output_tokens are populated
        cop_has_tokens = any(
            (r.get("input_tokens") or 0) + (r.get("output_tokens") or 0) > 0
            for r in cop_recent_list
        )
        copilot_focus = {
            "total_calls": cop_total,
            "successful": cop_successful,
            "failed": cop_failed,
            "blocked": cop_blocked,
            "by_runner": cop_by_runner,
            "top_tasks": cop_top_tasks,
            "recent_calls": cop_recent_list,
            "token_detail_available": cop_has_tokens,
            "token_detail_note": (
                "" if cop_has_tokens
                else "Copilot-Daemon output does not expose token stats"
            ),
        }

        # ── 6. Top tasks by total LLM calls ───────────────────────────────
        ...
        top_tasks = [dict(r) for r in top_task_rows]

        # ── 7. Recent calls (unified) ──────────────────────────────────────
        recent_rows = conn.execute(
            f"""SELECT
                    e.id, e.created_at, e.event_type,
                    {_ROLE_SQL} AS usage_role,
                    e.runner_type, e.provider, e.model,
                    e.agent_label, e.task_id,
                    t.title      AS task_title,
                    e.trigger_source, e.success, e.blocked,
                    e.block_reason, e.error, e.duration_ms,
                    e.input_tokens, e.output_tokens, e.cached_tokens,
                    e.total_tokens, e.premium_requests,
                    e.rate_limit_type, e.rate_limit_used_pct, e.caller_file
                FROM llm_audit_events e
                LEFT JOIN agent_tasks t ON t.id = e.task_id
                WHERE e.day_label=? AND {_RELEVANT}
                ORDER BY e.id DESC
        ...
        _COP_THRESHOLD = 10
        _TASK_THRESHOLD = 3
        _LONG_RUN_MS = 30 * 60 * 1000  # 30 min

        warnings = []
        if cop_total > _COP_THRESHOLD:
            warnings.append({"code": "COPILOT_USAGE_HIGH",
                              "message": f"Copilot usage high today ({cop_total} calls > {_COP_THRESHOLD})"})
        for t in cop_top_tasks:
            if t["calls"] > _TASK_THRESHOLD:
                warnings.append({"code": "TASK_COPILOT_REPEATED",
                                  "message": f"Task #{t['task_id']} has repeated Copilot calls ({t['calls']})"})
        if cop_failed > 0:
            warnings.append({"code": "COPILOT_FAILED",
                              "message": f"Failed Copilot calls detected ({cop_failed})"})
        if cop_blocked > 0:
            warnings.append({"code": "COPILOT_BLOCKED",
                              "message": f"Blocked Copilot attempts detected ({cop_blocked})"})
        if not cop_has_tokens and cop_total > 0:
            warnings.append({"code": "COPILOT_TOKEN_UNAVAILABLE",
                              "message": "Copilot token detail unavailable"})

        # Detect possible retry loops: same task claimed >_TASK_THRESHOLD times by worker/light_worker
        retry_rows = conn.execute(
            f"""SELECT task_id, COUNT(*) AS cnt
                FROM llm_audit_events
                WHERE day_label=? AND {_RELEVANT}
                    AND runner_type IN ('worker','light_worker')
        ...
        for rr in retry_rows:
            warnings.append({"code": "POSSIBLE_RETRY_LOOP",
                              "message": f"Possible retry loop on task #{rr['task_id']} ({rr['cnt']} attempts)"})

        # Long-running Copilot call
        long_cop = conn.execute(
            f"""SELECT id, task_id, duration_ms
                FROM llm_audit_events
                WHERE day_label=? AND {_RELEVANT}
                    AND duration_ms > ?
                    AND (runner_type='copilot-daemon' OR provider LIKE '%copilot%')
                ORDER BY duration_ms DESC
                LIMIT 1""",
            (dl, _LONG_RUN_MS),
        ).fetchone()
        if long_cop:
            mins = round(long_cop["duration_ms"] / 60000, 1)
            warnings.append({"code": "COPILOT_LONG_RUNNING",
                              "message": f"Long-running Copilot call on task #{long_cop['task_id']} ({mins} min)"})
...
def get_today_auto_monitor_by_dedupe_key(dedupe_key: str):
    """Return earliest non-FAILED AUTO-MONITOR task with given dedupe_key created today (Asia/Taipei), or None.
...
def get_today_task_by_dedupe_key(dedupe_key: str):
    """Return earliest non-FAILED task with given dedupe_key created today (Asia/Taipei), or None.
...
def get_oldest_stale_queued_research_task(min_age_seconds: int = 300):
    """Return oldest QUEUED research task older than min_age_seconds, or None."""
...
def has_claim_event_for_task(task_id: int) -> bool:
    """Return True if a claim event already exists for the given task_id."""
    if not task_id:
        return False
    conn = get_conn()
    try:
        row = conn.execute(
            ...
            WHERE task_id = ?
              AND outcome IN ('WORKER_CLAIMED', 'COPILOT_DAEMON_CLAIMED')
            LIMIT 1
            """,
            (task_id,),
        ).fetchone()
...
# ── LLM Planner multi-provider settings ──────────────────────────────────────

_VALID_PLANNER_LLM_PROVIDERS = {"local", "claude-cli", "codex-cli", "auto"}
_VALID_PLANNER_LLM_MODES = {"off", "suggest_only", "create_task", "create_and_route"}


def get_planner_llm_provider() -> str:
    value = get_setting("planner_llm_provider", DEFAULT_SETTINGS["planner_llm_provider"])
    v = str(value or "local").strip().lower()
    return v if v in _VALID_PLANNER_LLM_PROVIDERS else "local"


def set_planner_llm_provider(provider: str):
    v = str(provider or "local").strip().lower()
    if v not in _VALID_PLANNER_LLM_PROVIDERS:
        raise ValueError(f"Invalid planner_llm_provider: {provider!r}. Must be one of {sorted(_VALID_PLANNER_LLM_PROVIDERS)}")
    set_setting("planner_llm_provider", v)


def get_planner_llm_model() -> str:
    value = get_setting("planner_llm_model", DEFAULT_SETTINGS["planner_llm_model"])
    return str(value or "auto").strip() or "auto"


def set_planner_llm_model(model: str):
    v = str(model or "auto").strip() or "auto"
    set_setting("planner_llm_model", v)


def get_planner_llm_enabled() -> bool:
    value = get_setting("planner_llm_enabled", DEFAULT_SETTINGS["planner_llm_enabled"])
    return str(value).strip() in ("1", "true", "TRUE", "yes", "on")


def set_planner_llm_enabled(enabled: bool):
    set_setting("planner_llm_enabled", "1" if enabled else "0")


def get_planner_llm_mode() -> str:
    value = get_setting("planner_llm_mode", DEFAULT_SETTINGS["planner_llm_mode"])
    v = str(value or "off").strip().lower()
    return v if v in _VALID_PLANNER_LLM_MODES else "off"


def set_planner_llm_mode(mode: str):
    v = str(mode or "off").strip().lower()
    if v not in _VALID_PLANNER_LLM_MODES:
        raise ValueError(f"Invalid planner_llm_mode: {mode!r}. Must be one of {sorted(_VALID_PLANNER_LLM_MODES)}")
    set_setting("planner_llm_mode", v)


def get_planner_budget_guard_enabled() -> bool:
    value = get_setting("planner_budget_guard_enabled", DEFAULT_SETTINGS["planner_budget_guard_enabled"])
    return str(value).strip() in ("1", "true", "TRUE", "yes", "on")


def set_planner_budget_guard_enabled(enabled: bool):
    set_setting("planner_budget_guard_enabled", "1" if enabled else "0")


def get_planner_daily_call_cap() -> int:
    value = get_setting("planner_daily_call_cap", DEFAULT_SETTINGS["planner_daily_call_cap"])
    try:
...
def set_planner_daily_call_cap(cap: int):
    set_setting("planner_daily_call_cap", str(max(0, int(cap or 0))))


def get_planner_timeout_seconds() -> int:
    value = get_setting("planner_timeout_seconds", DEFAULT_SETTINGS["planner_timeout_seconds"])
    try:
        return max(30, int(value or 300))
    except (ValueError, TypeError):
        return 300


def set_planner_timeout_seconds(seconds: int):
    set_setting("planner_timeout_seconds", str(max(30, int(seconds or 300))))


def get_planner_max_output_chars() -> int:
    value = get_setting("planner_max_output_chars", DEFAULT_SETTINGS["planner_max_output_chars"])
    try:
        return max(1000, int(value or 12000))
    except (ValueError, TypeError):
        return 12000


def set_planner_max_output_chars(chars: int):
    set_setting("planner_max_output_chars", str(max(1000, int(chars or 12000))))


def get_planner_today_calls(day_label: Optional[str] = None) -> int:
    """Return count of LLM planner calls today (runner_type='planner', real providers only)."""
    from orchestrator.common import local_day_label
    label = day_label or local_day_label()
    conn = get_conn()
    try:
        row = conn.execute(
            "SELECT SUM(call_count) FROM llm_usage_events "
            "WHERE runner_type='planner' AND day_label=? AND provider NOT IN ('local','adaptive_regime','')",
            (label,),
        ).fetchone()
...
_VALID_CTO_REVIEW_PROVIDERS = {"local", "claude-cli", "codex-cli", "auto"}
_VALID_CTO_REVIEW_MODES = {"local_review", "review_only"}


def get_cto_review_provider() -> str:
    value = get_setting("cto_review_provider", DEFAULT_SETTINGS["cto_review_provider"])
    v = str(value or "local").strip().lower()
    return v if v in _VALID_CTO_REVIEW_PROVIDERS else "local"


def set_cto_review_provider(provider: str):
    v = str(provider or "local").strip().lower()
    if v not in _VALID_CTO_REVIEW_PROVIDERS:
        raise ValueError(f"Invalid cto_review_provider: {provider!r}. Must be one of {sorted(_VALID_CTO_REVIEW_PROVIDERS)}")
    set_setting("cto_review_provider", v)


def get_cto_review_model() -> str:
    value = get_setting("cto_review_model", DEFAULT_SETTINGS["cto_review_model"])
    return str(value or "auto").strip() or "auto"


def set_cto_review_model(model: str):
    set_setting("cto_review_model", str(model or "auto").strip() or "auto")


def get_cto_review_mode() -> str:
    value = get_setting("cto_review_mode", DEFAULT_SETTINGS["cto_review_mode"])
    v = str(value or "local_review").strip().lower()
    return v if v in _VALID_CTO_REVIEW_MODES else "local_review"


def set_cto_review_mode(mode: str):
    v = str(mode or "local_review").strip().lower()
    if v not in _VALID_CTO_REVIEW_MODES:
        raise ValueError(f"Invalid cto_review_mode: {mode!r}. Must be one of {sorted(_VALID_CTO_REVIEW_MODES)}")
    set_setting("cto_review_mode", v)


def get_cto_review_daily_call_cap() -> int:
    value = get_setting("cto_review_daily_call_cap", DEFAULT_SETTINGS["cto_review_daily_call_cap"])
    try:
        return max(0, int(value or 0))
    except (ValueError, TypeError):
        return 0
...
def set_cto_review_daily_call_cap(cap: int):
    set_setting("cto_review_daily_call_cap", str(max(0, int(cap or 0))))


def get_cto_review_timeout_seconds() -> int:
    value = get_setting("cto_review_timeout_seconds", DEFAULT_SETTINGS["cto_review_timeout_seconds"])
    try:
        return max(30, int(value or 300))
    except (ValueError, TypeError):
        return 300
...
def set_cto_review_timeout_seconds(seconds: int):
    set_setting("cto_review_timeout_seconds", str(max(30, int(seconds or 300))))


def get_cto_review_max_context_chars() -> int:
    value = get_setting("cto_review_max_context_chars", DEFAULT_SETTINGS["cto_review_max_context_chars"])
    try:
        return max(1000, int(value or 12000))
    except (ValueError, TypeError):
        return 12000
...
def set_cto_review_max_context_chars(chars: int):
    set_setting("cto_review_max_context_chars", str(max(1000, int(chars or 12000))))


def get_cto_review_max_output_chars() -> int:
    value = get_setting("cto_review_max_output_chars", DEFAULT_SETTINGS["cto_review_max_output_chars"])
    try:
        return max(500, int(value or 6000))
    except (ValueError, TypeError):
        return 6000


def set_cto_review_max_output_chars(chars: int):
    set_setting("cto_review_max_output_chars", str(max(500, int(chars or 6000))))


def get_cto_today_calls(day_label: Optional[str] = None) -> int:
    """Return count of CTO LLM review calls today (usage_role='cto', real providers only)."""
    from orchestrator.common import local_day_label
    label = day_label or local_day_label()
    conn = get_conn()
    try:
        row = conn.execute(
            "SELECT SUM(call_count) FROM llm_usage_events "
            "WHERE usage_role='cto' AND day_label=? AND provider NOT IN ('local','adaptive_regime','')",
            (label,),
        ).fetchone()
        return int(row[0] or 0) if row else 0
    except Exception:
