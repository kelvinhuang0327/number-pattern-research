...
METHOD_ALIASES = {
    ...
    'gs_big_bet1': 'gs_big_bet1_predict',
    'gs_big_bet2': 'gs_big_bet2_predict',
    'gs_big_bet3': 'gs_big_bet3_predict',
    'gs_big_bet4': 'gs_big_bet4_predict',
}
...
def get_method_func(engine, method_name):
    ...
    print(f"DEBUG Loopup: input={method_name}, actual={actual_name}")
    if 'smart' in method_name:
        glob_keys = [k for k in globals().keys() if 'smart' in k]
        print(f"DEBUG Globals matching 'smart': {glob_keys}")
    # print(f"DEBUG: method={method_name}, actual={actual_name}")
    # glob_keys = [k for k in globals().keys() if 'gs_' in k]
    # print(f"DEBUG: globals with gs_: {glob_keys}")
...
def backtest_single(method_name, test_periods=150):
    ...
    print("=" * 60)
    print(f"🔬 大樂透 {test_periods} 期回測（獨立腳本）")
    print("=" * 60)
...
def backtest_multi(method_names, test_periods=150):
    ...
    print("=" * 60)
    print(f"🔬 大樂透 {len(methods)} 注組合 {test_periods} 期回測（獨立腳本）")
    print("=" * 60)
    print(f"方法組合:")
...
def main():
    parser = argparse.ArgumentParser(description='大樂透 500 期回測（獨立腳本）')
...
# Grand Slam Big Lotto Strategy Implementation
def gs_big_bet1_predict(history, rules):
    engine = UnifiedPredictionEngine()
    alpha_rules = rules.copy()
    alpha_rules['pickCount'] = 18 # Total Alpha Pool
    alpha_rules['frequency_window'] = 50
    pred = engine.true_frequency_predict(history, alpha_rules)
    nums = pred.get('ranked_list', [])[:6] # Top 1-6
    return {'numbers': sorted(nums)}

def gs_big_bet2_predict(history, rules):
    engine = UnifiedPredictionEngine()
    alpha_rules = rules.copy()
    alpha_rules['pickCount'] = 18
    alpha_rules['frequency_window'] = 50
    pred = engine.true_frequency_predict(history, alpha_rules)
    nums = pred.get('ranked_list', [])[6:12] # Top 7-12
    return {'numbers': sorted(nums)}

def gs_big_bet3_predict(history, rules):
    engine = UnifiedPredictionEngine()
    alpha_rules = rules.copy()
    alpha_rules['pickCount'] = 18
    alpha_rules['frequency_window'] = 50
    pred = engine.true_frequency_predict(history, alpha_rules)
    nums = pred.get('ranked_list', [])[12:18] # Top 13-18
    return {'numbers': sorted(nums)}

def gs_big_bet4_predict(history, rules):
    engine = UnifiedPredictionEngine()
    beta_rules = rules.copy()
    beta_rules['pickCount'] = 6
    pred = engine.deviation_predict(history, beta_rules)
    return {'numbers': pred['numbers']}
