...
async def test_daily539_prediction_match():
    print('=' * 80)
    print('🎯 測試今彩539預測方法匹配度')
    print('=' * 80)
    print()
    print('目標號碼: 04, 07, 11, 16, 26')
    print()
    ...
    for method_name, method_type in prediction_methods:
        try:
            ...
            # 轉換為整數集合以便比較
            predicted_numbers = set(int(n) for n in result.get('numbers', []))
            matches = len(predicted_numbers & target_numbers)

            results.append({
                'name': method_name,
                'numbers': sorted([int(n) for n in result.get('numbers', [])]),
                'matches': matches,
                'confidence': result.get('confidence', 0)
            })

            match_str = ', '.join(f'{n:02d}' for n in sorted(predicted_numbers & target_numbers))
            numbers_str = ', '.join(f'{n:02d}' for n in sorted(predicted_numbers))
            print(f'📌 {method_name:15s} | 預測: [{numbers_str}] | 命中: {matches}/5 [{match_str or "無"}]')
    ...
    try:
        ...
        bet1_numbers = set(int(n) for n in opt_result['bet1']['numbers'])
        bet2_numbers = set(int(n) for n in opt_result['bet2']['numbers'])
        ...
        double_bet_results.append({
            'name': '優化集成雙注',
            'bet1': sorted([int(n) for n in opt_result['bet1']['numbers']]),
            'bet2': sorted([int(n) for n in opt_result['bet2']['numbers']]),
            'bet1_matches': bet1_matches,
            'bet2_matches': bet2_matches,
            'max_matches': max_matches
        })

        bet1_match_str = ', '.join(f'{n:02d}' for n in sorted(bet1_numbers & target_numbers))
        bet2_match_str = ', '.join(f'{n:02d}' for n in sorted(bet2_numbers & target_numbers))

        print(f'✅ 優化集成雙注')
        print(f'   第一注: [{", ".join(f"{n:02d}" for n in sorted(bet1_numbers))}] | 命中: {bet1_matches}/5 [{bet1_match_str or "無"}]')
        print(f'   第二注: [{", ".join(f"{n:02d}" for n in sorted(bet2_numbers))}] | 命中: {bet2_matches}/5 [{bet2_match_str or "無"}]')
    ...
    try:
        ...
        bet1_numbers = set(int(n) for n in bay_result['bet1']['numbers'])
        bet2_numbers = set(int(n) for n in bay_result['bet2']['numbers'])
        ...
        double_bet_results.append({
            'name': '貝葉斯優化雙注',
            'bet1': sorted([int(n) for n in bay_result['bet1']['numbers']]),
            'bet2': sorted([int(n) for n in bay_result['bet2']['numbers']]),
            'bet1_matches': bet1_matches,
            'bet2_matches': bet2_matches,
            'max_matches': max_matches
        })
    ...
    for idx, r in enumerate(results, 1):
        ...
        match_str = ', '.join(f'{n:02d}' for n in sorted(matched)) if matched else '無'
        medal = '🥇' if idx == 1 else '🥈' if idx == 2 else '🥉' if idx == 3 else '  '
        print(f'{medal} {idx}. {r["name"]:15s} | 命中 {r["matches"]}/5 | 命中號碼: {match_str}')
    ...
    for idx, r in enumerate(double_bet_results, 1):
        ...
        bet1_match_str = ', '.join(f'{n:02d}' for n in sorted(bet1_matched)) if bet1_matched else '無'
        bet2_match_str = ', '.join(f'{n:02d}' for n in sorted(bet2_matched)) if bet2_matched else '無'

        print(f'   第一注: [{", ".join(f"{n:02d}" for n in r["bet1"])}] | 命中 {r["bet1_matches"]}/5 | 命中號碼: {bet1_match_str}')
        print(f'   第二注: [{", ".join(f"{n:02d}" for n in r["bet2"])}] | 命中 {r["bet2_matches"]}/5 | 命中號碼: {bet2_match_str}')
    ...
    if results:
        ...
        if best_single["matches"] > 0:
            matched = set(best_single['numbers']) & target_numbers
            match_str = ', '.join(f'{n:02d}' for n in sorted(matched))
            print(f'   命中號碼: {match_str}')
...
if __name__ == '__main__':
    asyncio.run(test_daily539_prediction_match())
