...
BASELINE = {
    ...
    "p126f_count": 6000,
    # P131: DAILY_539 acb_markov_midfreq_3bet bet-2 + bet-3 Wave 2 controlled apply (2026-05-28)
    ...
    "p134_count": 3002,
    # P140: POWER_LOTTO power_precision_3bet bet-2 + bet-3 controlled apply (2026-05-29)
    "p140_apply_id": "P140_APPLY_POWER_PRECISION_3BET_v1",
    "p140_count": 3000,
    "total_count": 88924,  # Updated after RSR-6 cleanup: deleted 40 orphan bet_index=2 rows (2026-05-28)  # 61962 (pre-P126E) + 4500 (P126E bet-2+bet-3+bet-4) = 66462
}
...
def run_checks(db_path: pathlib.Path) -> dict:
    ...
    if p21b_count != BASELINE["p21b_count"]:
        violations.append(
    ...
    if p37_count != BASELINE["p37_count"]:
        violations.append(
    ...
    if p48_count != BASELINE["p48_count"]:
        violations.append(
