...
- Task worktrees should not accumulate after merge. The PR-open phase may retain a task worktree for audit/review, but the PR-merged phase must follow RUNBOOK `Post-Merge Isolated Worktree Cleanup Gate` after post-merge verification unless Owner override is present. Repo-internal `report/` artifacts are durable evidence and are not cleanup targets; broad `/Users/kelvin/Kelvin-WorkSpace` cleanup is a separate quarantine-first task, not normal PR merge cleanup.
