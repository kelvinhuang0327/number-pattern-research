...
PROTECTED_JSON_BYTE_SIZE = 11_129_904
PROTECTED_JSON_SHA256 = "abde59f03cd48841194fe19ec1f94c9f32997cb62fd549fff16eaf02f3d05255"
PROTECTED_MARKDOWN_BYTE_SIZE = 38_807
PROTECTED_MARKDOWN_SHA256 = "90da9ad3397aee0a98b69516467f023784277638db608ccb91e203ef22fbc777"
EXPECTED_CURRENT_MAIN_ADDITIONS = frozenset(
    {
        "analysis/p541a_biglotto_strategy_inventory_replay_coverage_audit.py",
        "analysis/p541b_biglotto_legacy_method_classification_audit.py",
        "analysis/p541b_r2_biglotto_legacy_method_classification_audit.py",
        "analysis/p541c_r2_biglotto_legacy_method_review_readiness_selection.py",
        "analysis/p541d_r2_biglotto_selected_method_adapter_design.py",
        "analysis/p544b_readonly_replay_artifact_inventory.py",
        "analysis/p544c_unified_lottery_replay_success_evaluator.py",
        "analysis/p545a_readonly_official_outcomes_registry_export.py",
        "analysis/p545b_full_50_300_750_per_draw_evaluator.py",
        "analysis/p545b_r2_full_per_draw_evaluator.py",
        "analysis/p545c_r4_strategy_draw_opportunity_registry.py",
        "lottery_api/models/p541d_r2_biglotto_selected_adapters.py",
        "lottery_api/models/strategy_preserving_20_ticket.py",
    }
)
AUTHORIZED_CHANGED_PATHS = frozenset(
    {
        "analysis/p541b_r2_biglotto_legacy_method_classification_audit.py",
        ...
        "outputs/research/p541b_r2_biglotto_legacy_method_classification_audit_20260711.md",
        "analysis/p541c_r2_biglotto_legacy_method_review_readiness_selection.py",
        "tests/test_p541c_r2_biglotto_legacy_method_review_readiness_selection.py",
        ...
        "analysis/p541d_r2_biglotto_selected_method_adapter_design.py",
        "tests/test_p541d_r2_biglotto_selected_method_adapter_design.py",
...
def _assert_json_self_identity_contract(
    ...
    *,
    ...
    assert not differences or (
        len(differences) == 2 and signature == expected_signature
...
def test_missing_current_main_manifest_entry_remains_terminal():
    mod = _module()
    with pytest.raises(mod.P541BR2Error, match="corpus incomplete"):
        mod.require_current_main_entries(["missing.py"], {})


def test_fresh_main_scope_cannot_silently_omit_new_matching_files(tmp_path):
    repo, commit = _synthetic_repo(
        tmp_path,
        {
            "tools/legacy.py": "VALUE = 1\n",
            "analysis/new_current_main_method.py": "BIG_LOTTO = True\n",
            "analysis/not_a_match.py": "VALUE = 2\n",
            "tests/test_new_current_main_method.py": "# biglotto\n",
            "lottery_api/models/replay_strategy_registry.py": "# 大樂透\n",
        },
    )
    historical = [
        {
            "method_id": "tools/legacy.py",
            "source_path": "tools/legacy.py",
            "discovery_group": "p541a_tools",
        }
    ]
    records, entries, scope = _module().discover_current_main_scope(
        repo, historical, commit
    )
    paths = [record["source_path"] for record in records]
    assert paths == [
        "analysis/new_current_main_method.py",
        "tools/legacy.py",
    ]
    assert set(entries) == set(paths)
    assert scope["historical_seed_count"] == 1
    assert scope["current_main_addition_count"] == 1
    assert scope["ordered_current_main_additions"] == [
        "analysis/new_current_main_method.py"
    ]
    assert scope["record_count"] == 2
    assert scope["verification"] == "PASS"
    assert scope["excluded_keyword_matches"] == [
        {
            "source_path": "lottery_api/models/replay_strategy_registry.py",
            "reason": "existing_registry_wiring",
        },
        {
            "source_path": "tests/test_new_current_main_method.py",
            "reason": "test_source",
        },
    ]


def test_fresh_main_scope_fails_closed_on_ambiguous_group_identity(
    ...
    monkeypatch.setattr(
        ...
        mod.CURRENT_MAIN_SCOPE_GROUPS
        + ({"name": "overlap", "kind": "prefix", "value": "analysis/"},),
    )
    historical = [
        {
            "method_id": "tools/legacy.py",
    ...
    with pytest.raises(mod.P541BR2Error, match="ambiguous current-main"):
        mod.discover_current_main_scope(repo, historical, commit)
...
def test_artifact_contract_is_complete(artifact):
    ...
    record_count = len(artifact["method_classification_records"])
    scope = artifact["provenance"]["fresh_main_scope"]
    assert artifact["implementation_base_oid"] == mod.BASE_MAIN_COMMIT
    assert artifact["current_main_source_commit"] == mod.BASE_MAIN_COMMIT
    assert artifact["frozen_source_commit"] == mod.BASE_MAIN_COMMIT
    assert artifact["summary"]["total_records"] == record_count
    assert scope["historical_seed_count"] == mod.HISTORICAL_RECORD_COUNT
    assert scope["record_count"] == record_count
    assert scope["current_main_addition_count"] == len(
        EXPECTED_CURRENT_MAIN_ADDITIONS
    )
    assert set(scope["ordered_current_main_additions"]) == EXPECTED_CURRENT_MAIN_ADDITIONS
    assert record_count == mod.HISTORICAL_RECORD_COUNT + len(
        EXPECTED_CURRENT_MAIN_ADDITIONS
    )
    assert len(artifact["provenance"]["source_manifest"]["ordered_entries"]) == record_count
    ...
    assert sum(artifact["summary"]["scan_status_counts"].values()) == record_count
    for counts in artifact["summary"]["evidence_status_counts"].values():
        assert set(counts) == set(mod.TRI_STATES)
        assert sum(counts.values()) == record_count
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("current_main_source_commit", "top-level provenance"),
    ],
)
...
def test_blob_read_failure_retains_order_continues_and_is_byte_deterministic(
    ...
    assert sum(first["summary"]["scan_status_counts"].values()) == len(
        first["method_classification_records"]
    )
...
def test_branch_diff_contains_only_authorized_paths():
    mod = _module()
    assert mod.BASE_MAIN_COMMIT == "e0f6c26f2c15d8b1e75ae1eb8df5b5aa97977a65"
