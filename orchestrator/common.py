...
# ── LLM Agent Label normalisation ─────────────────────────────────────────────
_LLM_PROVIDER_DISPLAY: dict = {
    "claude":          "Claude",
    "claude-cli":      "Claude",
    "codex":           "Codex",
    "codex-cli":       "Codex",
    ...
    "copilot":         "Copilot",
    "local":           "Local",
    "adaptive_regime": "Local",
}


def normalize_llm_agent_label(
    provider: str,
    model: str = "",
    runner_type: str = "",
) -> dict:
    """
    Normalise provider + model into a human-readable display label for LLM attribution.

    Returns a dict:
        display_label  — e.g. "Copilot-Daemon claude-sonnet-4-5" or "Claude" or "Local"
        provider_label — e.g. "Copilot-Daemon"
        model_label    — model string, or "" when unknown/absent

    Rules:
        - local / adaptive_regime / ""    → "Local" (no real LLM call)
        - known provider + model present  → "{Provider} {model}"
        - known provider, no model        → "{Provider}"
        - unknown / unrecognised provider → capitalised provider string
    """
    provider_clean = str(provider or "").strip().lower()
    model_clean = str(model or "").strip()

    if not provider_clean or provider_clean in ("local", "adaptive_regime"):
        return {"display_label": "Local", "provider_label": "Local", "model_label": ""}

    provider_label = _LLM_PROVIDER_DISPLAY.get(provider_clean, provider_clean.capitalize())
    model_label = model_clean  # empty string preserved

    display_label = f"{provider_label} {model_clean}" if model_clean else provider_label
...
def dedupe_day_utc() -> str:
    ...
    Deprecated for daily cap / dedupe_key building — use local_day_label() instead,
    which returns the Asia/Taipei date and correctly resets the cap at Taiwan midnight
    (00:00 CST = 16:00 UTC prior day).
    """
    return datetime.now(timezone.utc).strftime("%Y-%m-%d")


def local_day_label(tz: str = "Asia/Taipei") -> str:
    """Return today's date in Asia/Taipei local time as YYYY-MM-DD.

    Use this for all Planner daily cap / dedupe key building instead of
    dedupe_day_utc().  Ensures Taiwan midnight (00:00 CST = 16:00 UTC prior day)
    correctly resets the daily cap rather than waiting until 08:00 Taiwan time.
    """
