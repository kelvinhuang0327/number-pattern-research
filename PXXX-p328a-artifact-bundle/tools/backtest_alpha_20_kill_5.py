#!/usr/bin/env python3
"""
Alpha 20 Strategy Rolling Backtest - Kill 5 (Conservative)
目標：驗證殺 5 碼是否比殺 10 碼或不殺號有更好的績效平衡
"""
import sys
import os
import io
...
class Alpha20BacktesterKill5:
    ...
    def run(self, year=2025):
        ...
        print(f"🚀 Alpha 20 基於 Kill-5 (保守殺號) 回測啟動 (年份: {year})")
        print("-" * 80)
        ...
        for i, target_draw in enumerate(test_draws):
            ...
            # 1. Kill 5 Numbers
            kill_nums = self.selector.predict_kill_numbers(count=5)
            ...
            # 3. Bet 2: Zone Balance 500 + Filter search
            bet2 = []
            for attempt in range(20):
                sub_hist = history[-(500 + attempt):]
                if attempt > 0: sub_hist = sub_hist[:-attempt]
                try:
...
def main():
    backtester = Alpha20BacktesterKill5()
    backtester.run(2025)
