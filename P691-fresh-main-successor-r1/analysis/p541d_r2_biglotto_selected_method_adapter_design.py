...
BASE_COMMIT = "e0f6c26f2c15d8b1e75ae1eb8df5b5aa97977a65"
TASK_ID = "P691_FRESH_MAIN_SUCCESSOR_R1_P541D_CHAIN_REPAIR"
SCHEMA_VERSION = "p541d-r2-adapter-design-v1"
DESIGNER_VERSION = "p541d-r2-designer-v3-fresh-main-r1"
...
UPSTREAM_IDENTITY = {
    "path": UPSTREAM_PATH,
    "byte_size": 1_440_823,
    "sha256": "e0d0d087d77d218d321142faf74ae9432f4c8f7a0ec73eb073f5ff9d4532935a",
...
def validate_upstream(upstream: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not isinstance(records, list) or len(records) != UPSTREAM_IDENTITY["record_count"]:
        raise P541DR2ValidationError(
            f"P541C_R2 must contain exactly {UPSTREAM_IDENTITY['record_count']} reviewed records"
        )
...
def _load_upstream(repo_root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    artifact_path = (repo_root / UPSTREAM_PATH).resolve()
    expected_path = (REPO_ROOT / UPSTREAM_PATH).resolve()
    if artifact_path != expected_path:
        raise P541DR2ValidationError("P541C_R2 must be read from the exact allowlisted artifact path")
    ...
    return upstream, {
        ...
        **actual,
        "verification": "PASS",
        "read_mechanism": "allowlisted regenerated working-tree artifact",
    }
...
def build_packet(repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    packet = {
        ...
        "no_db_no_execution_evidence": {
            "upstream_read_mechanism": "allowlisted regenerated artifact with exact byte/SHA gate",
