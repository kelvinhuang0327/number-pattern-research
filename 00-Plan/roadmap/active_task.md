...
> **STATUS: `P258B_READ_ONLY_PREREGISTRATION_CANDIDATE_SELECTED` → next `P258C` D3 read-only pre-registration design (strong model required)**
> **P258B (done):** External-response evaluation — exactly 3 directions received and adjudicated. D2 `DrawSetGeometryResidualConformal` HARD_REJECT (rule 11 — L82/L91/L73/L104/L105 set-geometry re-proposal, no survival argument). D1 `CrossLotteryLaggedEntropyRegime` REJECT_INSUFFICIENT_EVIDENCE (missing P256A boundary, L106 cross-lottery NULL, L86/L89 overfit risk). D3 `AdversarialNullSurvivorGate` ACCEPT_FOR_READ_ONLY_PREREGISTRATION — selected as at-most-one candidate. **Mandatory caveat: D3 is a validation/adversarial-null survivor gate, NOT a number-prediction method or production edge.** 40/40 P258B tests PASS; 22/22 P258A regression PASS. No DB write. No prototype. No registry/recommendation/production change. Human-gate fork: if CEO requires a predictive-signal survivor, classify as `P258B_NO_ELIGIBLE_EXTERNAL_DIRECTION` instead.
> **P258A (done):** Prediction-accuracy-only research intake protocol (read-only). Artifacts: `outputs/research/p258a_prediction_accuracy_research_intake_protocol_20260608.{json,md}`. 22/22 tests PASS.
> **P258-PRE0 (done):** Worktree disposition + CURRENT_STATE draw-total reconcile (64,361→64,366; legit post-draw backfill per `ingest_log.jsonl`, NOT data drift). PR #371 merged; main `96a5175`. DB 94,924 unchanged, integrity ok.
> **Next:** `P258C` — D3 `AdversarialNullSurvivorGate` read-only pre-registration design (Type B artifact, **strong model required**). No prototype. No DB write. No recommendation/production/registry change.
>
