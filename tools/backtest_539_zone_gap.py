...
"""
=============================================================================
539 3bet Zone-Gap 回測比較
=============================================================================
測試問題：新策略 (SumRange + GapPressure + ZoneShift) 的 3注結構，
         M2+ 率是否優於舊策略 (SumRange + Bayesian + ZoneBalance)
         且有統計顯著 Edge 超越 random 3-bet baseline？
...
def predict_3bet_new(hist):
    """NEW: SumRange + GapPressure + ZoneShift"""
...
def predict_3bet_old(hist):
    """OLD: SumRange + Bayesian + ZoneBalance"""
    engine = _get_engine()
    bets = []
...
def predict_random_3bet(seed_val):
    """Random 3-bet baseline factory — 3注隨機不重疊"""
    def _predict(hist):
...
def mcnemar_new_vs_old(all_draws, test_periods=500, seed=42, min_train=300):
    """McNemar's test: NEW 3-bet vs OLD 3-bet, per-draw paired comparison."""
    random.seed(seed)
    np.random.seed(seed)
    ...
    for i in range(test_periods):
        ...
        try:
            new_bets = predict_3bet_new(hist)
...
def main():
    ...
    report = {
        ...
        'strategy_new': 'SumRange + GapPressure + ZoneShift',
    ...
    # ── 1. Three-Window Stability (NEW strategy) ─────────────────
    print("\n" + "="*60)
    print("  [1] THREE-WINDOW STABILITY TEST (NEW)")
    print("="*60)
    tw_new = three_window_test(predict_3bet_new, all_draws, seed=42, min_train=300)
    report['three_window_new'] = {}
    for period, data in tw_new.items():
        ...
        report['three_window_new'][str(period)] = {
    ...
    # ── 3. Permutation Test (NEW vs Random) ──────────────────────
    print("\n" + "="*60)
    print("  [3] PERMUTATION TEST (NEW vs Random)")
    print("="*60)
    perm = permutation_test(predict_3bet_new, all_draws, test_periods=500, n_perms=200, seed=42, min_train=300)
    ...
    # ── 4. McNemar (NEW vs OLD) ──────────────────────────────────
    print("\n" + "="*60)
    print("  [4] McNEMAR TEST (NEW vs OLD)")
    print("="*60)
    mcn = mcnemar_new_vs_old(all_draws, test_periods=500, seed=42, min_train=300)
    report['mcnemar'] = mcn
    print(f"  NEW M2+ rate: {mcn['new_rate']*100:.2f}%")
    print(f"  OLD M2+ rate: {mcn['old_rate']*100:.2f}%")
    print(f"  New only: {mcn['new_only']}  Old only: {mcn['old_only']}")
    ...
    pass_criteria = {
        ...
        'new_beats_old': mcn['new_advantage'] > 0,
    }

    # Three-window stability: all windows must have positive M2+ edge
    tw_stable = all(
        tw_new.get(p, {}).get('ge2_edge', -1) > 0
    ...
    if all_pass:
        print(f"\n  ✅ DECISION: PASS — 新策略通過所有驗證，可部署")
    else:
        print(f"\n  ❌ DECISION: FAIL — 新策略未通過驗證，需回退")
    ...
    # ── Save Report ──────────────────────────────────────────────
    out_path = os.path.join(_base, '..', 'backtest_539_zone_gap_results.json')
