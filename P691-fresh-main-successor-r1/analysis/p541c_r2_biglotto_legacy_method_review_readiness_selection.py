...
INPUT_IDENTITY = {
    ...
    "byte_size": 11_129_904,
    "sha256": "abde59f03cd48841194fe19ec1f94c9f32997cb62fd549fff16eaf02f3d05255",
    "schema_version": "p541b-r2-evidence-v1",
    "detector_version": "p541b-r2-detector-v10-fresh-main-r1",
    "source_manifest_sha256": "f157fb05d88ebd28efd4bf3a5a97f880bbc8cdd56a559c688be72620b9aca63d",
    "record_count": 593,
}
SOURCE_MANIFEST_SHA256 = "f157fb05d88ebd28efd4bf3a5a97f880bbc8cdd56a559c688be72620b9aca63d"
SOURCE_MANIFEST_COUNT = 593
IMPLEMENTATION_BASE_COMMIT = "e0f6c26f2c15d8b1e75ae1eb8df5b5aa97977a65"
GENERATED_AT = "2026-07-19T10:20:38+00:00"
...
SHORTLIST_MAX = 20
EXPECTED_SUCCESSOR_SHORTLIST_PATHS = (
...
SELECTOR_VERSION = "p541c-r2-selector-v5-fresh-main-r1"
FINALIZATION_TASK_ID = "P691_FRESH_MAIN_SUCCESSOR_R1_P541C_CHAIN_REPAIR"
...
def validate_upstream_contract(p541b_r2: dict[str, Any]) -> None:
    ...
    if not isinstance(records, list) or len(records) != SOURCE_MANIFEST_COUNT:
        raise P541CR2ValidationError(
            f"P541B_R2 must contain exactly {SOURCE_MANIFEST_COUNT} method records"
        )
...
def validate_artifact(artifact: dict[str, Any]) -> None:
    ...
    repair = artifact.get("fresh_main_chain_repair")
    if (
        not isinstance(repair, dict)
        or repair.get("task_id") != "P691_FRESH_MAIN_SUCCESSOR_R1"
        or repair.get("base_commit") != IMPLEMENTATION_BASE_COMMIT
        or repair.get("selection_result")
        != "reproduced_exact_five_from_regenerated_evidence"
        or repair.get("current_contract")
        != {
            "detector_version": INPUT_IDENTITY["detector_version"],
            "record_count": SOURCE_MANIFEST_COUNT,
            "source_manifest_sha256": SOURCE_MANIFEST_SHA256,
        }
    ):
        raise P541CR2ValidationError("fresh-main chain repair mismatch")
...
def build_artifact(
    ...
    shortlist_paths = tuple(item["source_path"] for item in shortlist)
    if shortlist_paths != EXPECTED_SUCCESSOR_SHORTLIST_PATHS:
        raise P541CR2ValidationError(
            "P691 fresh-main shortlist drift: "
            f"expected={EXPECTED_SUCCESSOR_SHORTLIST_PATHS!r} actual={shortlist_paths!r}"
        )
    ...
    artifact = {
        ...
        "fresh_main_chain_repair": {
            "task_id": "P691_FRESH_MAIN_SUCCESSOR_R1",
            "base_commit": IMPLEMENTATION_BASE_COMMIT,
            "prior_stale_contract": (
                "P541C pinned detector-v4, the historical 580-record manifest, and the "
                "superseded P541B artifact bytes."
            ),
            "current_contract": {
                "detector_version": INPUT_IDENTITY["detector_version"],
                "record_count": SOURCE_MANIFEST_COUNT,
            ...
            "selection_result": "reproduced_exact_five_from_regenerated_evidence",
        },
...
def render_markdown(artifact: dict[str, Any]) -> str:
    ...
    lines.append("")
    lines.append("## Fresh-Main Chain Repair")
    lines.append("")
    repair = artifact["fresh_main_chain_repair"]
    lines.append(f"- task_id: `{repair['task_id']}`")
    lines.append(f"- base_commit: `{repair['base_commit']}`")
    lines.append(f"- prior_stale_contract: {repair['prior_stale_contract']}")
    lines.append(f"- selection_result: `{repair['selection_result']}`")
    lines.append("")
