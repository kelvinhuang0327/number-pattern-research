...
## ⚠️ OFFICIAL STATUS (2026-05-05) — VALID_SIGNAL_NON_MONETIZABLE

> **Research Closure (P1-03 — 2026-05-05)**
>
> - **Draw process**: CONSISTENT_WITH_UNIFORM (no exploitable bias)
> - **H6 long-window edge**: +4.00pp at 3000p OOS — CONFIRMED (p<0.001)
> - **EV verdict**: EV_NEGATIVE_BY_DESIGN — ROI = −81.29% structural; H6 edge covers only 1.2% of deficit
> - **M2 trap**: 91.19% of M2+ hits are break-even (M2 payout = ticket cost = 50 NTD)
> - **Strategy status**: H6_gate_mk20→ew85 is VALID_SIGNAL_NON_MONETIZABLE
> - **Betting for profit**: NOT RECOMMENDED
> - **New strategy mining**: FORBIDDEN (signal space exhausted: H001–H013 all REJECT)
> - **Allowed**: H6 live monitoring, watchdog alerts, framework-transfer research
> - **Reference**: outputs/research_closure_report.md, wiki/system/strategy_retirement_policy.md
...
## 2026-04-29 H6 Live Monitoring Integration

- **Active strategy**: `H6_gate_mk20→ew85` (VALIDATED, edge_150p=7.13%, sharpe=1.5)
- **Shadow strategy**: `acb_markov_midfreq_3bet` (rollback target)
- **Planner focus**: `MONITOR`
- **Live monitoring module**: `lottery_api/engine/h6_live_monitor.py`
- **DB**: `active_strategy_state` table, columns: `live_30p_edge`, `live_50p_edge`, `consecutive_negative_30p`, `current_regime`, `bear_regime_warning`, `rollback_triggered`, `rollback_status`, `rollback_reason`
- **Rollback triggers**: `live_30p_edge < -2%` OR `consecutive_negative_30p >= 5` OR `(bear_regime AND live_30p_edge < -1.93%)`
- **Rollback effect**: active → `acb_markov_midfreq_3bet`, shadow → `H6_gate_mk20→ew85`, planner_focus → `ROLLBACK_FOLLOWUP`
- **API endpoints**: `GET /api/h6-monitoring`, `POST /api/h6-monitoring/update`, `GET /api/h6-monitoring/rollback-simulate`
- **Prediction response**: includes `live_monitoring` block and `rollback_status` in every `/api/predict-coordinator` call
