...
export class OrchestrationManager {
  ...
  async _loadLlmControlPanel() {
    ...
    try {
      ...
      el.innerHTML = `
        ...
        <div class="llm-ctrl-section">
          <button class="llm-ctrl-colbtn" id="orc-evidence-toggle-btn">
            <span class="llm-ctrl-arrow">▶</span> Evidence Collector v0.1
          </button>
          <div id="orc-evidence-body" style="display:none">
            <div class="llm-ctrl-body">
              <div style="margin-bottom:4px">狀態: <span id="orc-evidence-status" style="font-weight:600">載入中…</span></div>
              <div id="orc-evidence-summary" style="font-size:0.8rem;color:var(--text-muted,#8b949e)">載入中…</div>
              <pre id="orc-evidence-items" style="font-size:0.72rem;margin-top:6px;max-height:200px;overflow:auto;background:var(--bg-secondary,#161b22);padding:6px;border-radius:4px"></pre>
            </div>
          </div>
        </div>
      `;
      ...
      _toggleCollapsible('orc-evidence-toggle-btn', 'orc-evidence-body');
  ...
  async _loadSummary() {
    try {
      ...
      // ── Evidence Collector v0.1 ───────────────────────────────────────────
      this._loadEvidenceStatus();
    } catch (e) {
  ...
  // Evidence Collector v0.1
  async _loadEvidenceStatus() {
    try {
      const r = await fetch('/api/orchestrator/evidence/status');
      const data = await r.json();
      const items = data.items || [];
      const topSev = items.length
        ? items.map(i => i.severity).sort((a,b) => ['CRITICAL','HIGH','MEDIUM','LOW'].indexOf(a) - ['CRITICAL','HIGH','MEDIUM','LOW'].indexOf(b))[0]
        : 'N/A';
      const taskTypes = [...new Set(items.map(i => i.suggested_task_type))].join(', ') || 'none';
      document.getElementById('orc-evidence-status').textContent = items.length > 0 ? 'HAS_EVIDENCE' : 'OK';
      document.getElementById('orc-evidence-summary').textContent =
        `Sources: rollback_false_positive / llm_usage_high / missing_outcomes | ` +
        `Count: ${items.length} | Top severity: ${topSev} | Suggested: ${taskTypes} | Requires LLM: false`;
      const el = document.getElementById('orc-evidence-items');
      if (el) el.textContent = JSON.stringify(items, null, 2);
    } catch(e) { console.error('evidenceStatus error', e); }
  }
