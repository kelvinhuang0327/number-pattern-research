#!/usr/bin/env python3
"""
P122: Trigger Recheck With Cross-Project Contamination Guard
Read-only script. Re-evaluates P121 trigger conditions against current DB state.
Explicitly guards against cross-project contamination from Betting-pool or other projects.
...
P121_ARTIFACT = PROJECT_ROOT / "outputs" / "replay" / "p121_trigger_recheck_20260527.json"
...
def check_contamination(operator_input: str = "") -> bool:
    """Return True if contamination detected (rejected context keywords present)."""
...
def build_artifact(replay_rows, db_rows, p108_count, p117_new_draws, p118_auth,
                   provenance_found, contamination_detected, p121_data):
    ...
    if contamination_detected:
        classification = "P122_BLOCKED_BY_CONTEXT_CONTAMINATION"
    ...
    if classification == "P122_ALL_TRIGGERS_STILL_BLOCKED":
        recommendation = (
            f"All P121 triggers remain BLOCKED. No change since P121. "
    ...
    return {
        "task_id": "P122_TRIGGER_RECHECK_CONTAMINATION_GUARD",
        "classification": classification,
        "p121_reference": {
            "classification": p121_data.get("classification", "P121_ALL_TRIGGERS_STILL_BLOCKED"),
            "artifact_path": "outputs/replay/p121_trigger_recheck_20260527.json",
        },
        "db_writes": False,
        ...
        "limitations": [
            ...
            "P108 count uses P99 cutoff draw 115000024; if cutoff changes the count will differ",
            "No change detected since P121: Special3=63, POWER_LOTTO new draws=0",
            "Contamination check is keyword-based on operator_input only; does not scan staged files",
        ],
...
def main():
    parser = argparse.ArgumentParser(description="P122 Trigger Recheck Contamination Guard")
    ...
    print("=== P122: Trigger Recheck / Cross-Project Contamination Guard ===")
    ...
    p121_data = {}
    if P121_ARTIFACT.exists():
        with open(P121_ARTIFACT) as f:
            p121_data = json.load(f)
    ...
    print()
    print(f"Priority trigger : {'NONE' if classification == 'P122_ALL_TRIGGERS_STILL_BLOCKED' else classification}")
    print(f"Classification   : {classification}")

    artifact = build_artifact(replay_rows, db_rows, p108_count, p117_new_draws,
                               p118_auth, provenance_found, contamination_detected, p121_data)
