...
> **STATUS: `P258C_D3_READ_ONLY_PREREGISTRATION_DESIGN_READY` → next `P258D` read-only implementation plan (NOT prototype)**
> **P258C (done):** D3 `AdversarialNullSurvivorGate` read-only pre-registration design. Artifacts: `outputs/research/p258c_d3_adversarial_null_gate_preregistration_20260608.{json,md}`. Matched adversarial-null family (M≥1000, 8 matching dims, per-draw Binomial null per L96), provenance+leakage gates, chronological OOS, short/mid/long (100/125/150 · 500/750/1000 · 1500-capped), primary endpoints = paired win rate vs P257A + candidate percentile vs matched null, BH-FDR+Bonferroni family, 6 risk-control triggers. **Falsification-only: rejects or marks not-yet-rejected, NEVER promotes; not a predictor; no accuracy claim; not an auto-approval gate.** 26/26 P258C tests PASS; 40/40 P258B + 22/22 P258A regression PASS. No DB write. No prototype. No registry/recommendation/production change.
> **P258B (done):** External-response evaluation — D2 HARD_REJECT (L82/L91/L73/L104/L105), D1 REJECT_INSUFFICIENT_EVIDENCE (P256A boundary missing, L106 NULL, L86/L89 overfit), D3 `AdversarialNullSurvivorGate` ACCEPT — selected. PR #373 merged. Human-gate fork: if CEO requires a predictive-signal survivor, `P258B_NO_ELIGIBLE_EXTERNAL_DIRECTION` instead.
...
> **Next:** `P258D` — D3 gate read-only IMPLEMENTATION PLAN only (module boundaries, test plan, data contracts). **No executable gate, no backtest, no DB write.** An executable prototype requires separate explicit authorization.
>
