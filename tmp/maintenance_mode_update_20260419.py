...
def phase2_exit_triggers() -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    for game in ["BIG_LOTTO", "DAILY_539", "POWER_LOTTO"]:
        ...
        edge150 = (sum(hits[-150:]) / 150 - baseline) if len(hits) >= 150 else (sum(hits) / max(len(hits), 1) - baseline)
        ...
        edge1500 = (sum(hits[-1500:]) / 1500 - baseline) if len(hits) >= 1500 else (sum(hits) / max(len(hits), 1) - baseline)
...
def mc_perm_conditional_edge(hits: List[int], baseline: float, n_perm: int = 500, seed: int = 42) -> Tuple[float, float]:
...
def phase3_stage2_recalibration() -> Dict[str, Any]:
    ...
    if os.path.exists(prev_path):
        try:
            ...
            old_thresholds = {k: v.get("optimal_threshold", 0.0) for k, v in prev.get("by_lottery", {}).items()}
        except Exception:
            old_thresholds = {}
...
def phase5_summary(
    ...
    summary = {
        ...
        "action_items": [
            {
                "priority": "HIGH",
                "action": f"Watch combo_B milestone draw {combo['evaluate_at_draw']} (expected around {expected_date}).",
            },
            {
                "priority": "MED",
