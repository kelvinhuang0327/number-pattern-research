...
def is_strategy_like(text: str) -> bool:
    if not text:
        return False
    lowered = text.lower()
    if any(sep in lowered for sep in ["/", ":", "|"]) or lowered.endswith((".json", ".md", ".py", ".js", ".cjs", ".txt")):
        return False
    if " vs " in lowered or "monitor" in lowered or "rolling_monitor" in lowered:
        return False
    if lowered in {"big lotto", "power lotto", "daily 539", "big_lotto", "power_lotto", "daily_539", "biglotto", "powerlotto", "daily539"}:
        return False
...
def infer_lottery_from_text(text: str) -> str:
    joined = text.lower()
    if "power_lotto_special" in joined or "power_ball" in joined or "power_lotto_bonus" in joined:
        return "POWER_LOTTO"
    if "big_lotto_bonus" in joined:
        return "BIG_LOTTO"
...
def pick_lottery(votes: Sequence[str], strategy_id: str, source_paths: Sequence[str]) -> str:
    if votes:
        ...
        if len(best) == 1 or best[0][1] > best[1][1]:
            candidate = best[0][0]
            if candidate in LOTTERY_ORDER:
                return candidate
            if "POWER_LOTTO" in candidate:
                return "POWER_LOTTO"
            if "BIG_LOTTO" in candidate:
                return "BIG_LOTTO"
            if "DAILY_539" in candidate or "539" in candidate:
                return "DAILY_539"
            return "UNSPECIFIED"
    ...
    if inferred not in LOTTERY_ORDER:
        if "POWER_LOTTO" in inferred:
            return "POWER_LOTTO"
        if "BIG_LOTTO" in inferred:
            return "BIG_LOTTO"
        if "DAILY_539" in inferred or "539" in inferred:
            return "DAILY_539"
        return "UNSPECIFIED"
    return inferred
...
def open_json(path: Path):
    ...
    try:
        ...
        try:
            ...
            pass
        # Some research artifacts are JSON-adjacent with comments / trailing commas.
        # Relax them just enough for read-only inventory parsing.
        stripped_lines = []
...
def collect_rejected(entries: Dict[str, EntryState]) -> Set[str]:
    ...
    for path in sorted(rejected_root.glob("*.json")):
        ...
        display = (
            re.search(r'"strategy"\s*:\s*"([^"]+)"', text)
            or re.search(r'"name"\s*:\s*"([^"]+)"', text)
            or re.search(r'"failure_reason"\s*:\s*"([^"]+)"', text)
            or re.search(r'"reason"\s*:\s*"([^"]+)"', text)
        )
        display = display.group(1) if display else raw_id
        lottery_m = re.search(r'"lottery"\s*:\s*"([^"]+)"', text)
        lottery = lottery_m.group(1) if lottery_m else infer_lottery_from_text(raw_id + " " + text)
        attach(
            entries,
            sid,
            display_name=display,
        ...
        if i <= 107 or any(key in line for key in ["REJECT", "WATCH", "OBSERVE", "PROVISIONAL", "OFFLINE", "RETIRED", "生產策略", "production", "現役"]):
            for tok in tokens:
                if not tok or tok in {"M3+", "M2+", "OOS", "Edge", "p"}:
                    continue
                if len(tok.split()) == 1 and tok.lower() not in LESSON_ALIAS_HINTS and not re.search(r"[_\-0-9]", tok):
                    continue
                if any(bad in tok.lower() for bad in ["grid_search", "report", "analysis", "benchmark", "study"]):
                    continue
