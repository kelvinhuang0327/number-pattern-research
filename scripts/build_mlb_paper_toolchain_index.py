#!/usr/bin/env python3
"""Build a deterministic local index for committed paper toolchain artifacts."""
from __future__ import annotations

import argparse
import sys
from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from wbc_backend.recommendation.paper_toolchain_index import (  # noqa: E402
    DEFAULT_DASHBOARD_HTML,
    DEFAULT_DASHBOARD_SECTIONS_CSV,
    DEFAULT_DASHBOARD_SUMMARY_JSON,
    DEFAULT_GATE_SUMMARY_JSON,
    DEFAULT_GENERATED_AT_UTC,
    DEFAULT_OUTPUT_DIR,
    DEFAULT_TOOLCHAIN_STATUS_JSON,
    PaperToolchainIndexError,
    build_paper_toolchain_index,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Render a result-only static index from committed P246/P247/P248 paper artifacts."
    )
    parser.add_argument("--dashboard-summary-json", type=Path, default=DEFAULT_DASHBOARD_SUMMARY_JSON)
    parser.add_argument("--dashboard-sections-csv", type=Path, default=DEFAULT_DASHBOARD_SECTIONS_CSV)
    parser.add_argument("--dashboard-html", type=Path, default=DEFAULT_DASHBOARD_HTML)
    parser.add_argument("--toolchain-status-json", type=Path, default=DEFAULT_TOOLCHAIN_STATUS_JSON)
    parser.add_argument("--gate-summary-json", type=Path, default=DEFAULT_GATE_SUMMARY_JSON)
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    parser.add_argument(
        "--generated-at-utc",
        default=DEFAULT_GENERATED_AT_UTC,
        help="fixed ISO timestamp for deterministic outputs",
    )
    parser.add_argument("--quiet", action="store_true", help="suppress completion message")
    return parser


def main(argv: list[str] | None = None) -> int:
    parser = build_parser()
    args = parser.parse_args(argv)
    try:
        result = build_paper_toolchain_index(
            dashboard_summary_json=args.dashboard_summary_json,
            dashboard_sections_csv=args.dashboard_sections_csv,
            dashboard_html=args.dashboard_html,
            toolchain_status_json=args.toolchain_status_json,
            gate_summary_json=args.gate_summary_json,
            output_dir=args.output_dir,
            generated_at_utc=args.generated_at_utc,
        )
    except PaperToolchainIndexError as exc:
        print(f"Paper toolchain index failed: {exc}", file=sys.stderr)
        return 2

    if not args.quiet:
        summary = result.summary
        print(
            f"Index {summary['index_status']}: "
            f"dashboard {summary['dashboard_status']}; "
            f"toolchain {summary['toolchain_status']}; "
            f"latest gate {summary['latest_gate_status']}; "
            f"{summary['local_link_count']} / {summary['link_count']} local links present"
        )
    return 1 if result.summary["index_status"] == "FAIL" else 0


if __name__ == "__main__":
    raise SystemExit(main())
