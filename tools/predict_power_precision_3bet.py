#!/usr/bin/env python3
"""
威力彩 Power Precision 3注預測
=============================
策略組成:
  注1: Fourier Rhythm (window=500) - 最優週期
  注2: Fourier Rhythm (window=500) - 次優週期
  注3: Lag-2 Echo Boost + Cold Numbers - 補償策略

驗證結果 (N=1384):
  - 150期 Edge: +3.50% (STABLE)
  - 500期 Edge: +2.43% (STABLE)
  - 1384期 Edge: +2.49% (STABLE)
  - 邊際增量 (vs 2注): +0.54% (三窗口全正)
"""
import os
import sys
import numpy as np
from collections import Counter
from scipy.fft import fft, fftfreq

project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, project_root)
sys.path.insert(0, os.path.join(project_root, 'lottery_api'))

from lottery_api.database import DatabaseManager

def get_fourier_rank(history, window=500):
    h_slice = history[-window:] if len(history) >= window else history
    w = len(h_slice)
    max_num = 38
    bitstreams = {i: np.zeros(w) for i in range(1, max_num + 1)}
    for idx, d in enumerate(h_slice):
        for n in d['numbers']:
            if n <= max_num: bitstreams[n][idx] = 1
    scores = np.zeros(max_num + 1)
    for n in range(1, max_num + 1):
        bh = bitstreams[n]
        if sum(bh) < 2: continue
        yf = fft(bh - np.mean(bh))
        xf = fftfreq(w, 1)
        idx_pos = np.where(xf > 0)
        pos_xf = xf[idx_pos]
        pos_yf = np.abs(yf[idx_pos])
        peak_idx = np.argmax(pos_yf)
        freq_val = pos_xf[peak_idx]
        if freq_val == 0: continue
        period = 1 / freq_val
        last_hit = np.where(bh == 1)[0][-1]
        gap = (w - 1) - last_hit
        scores[n] = 1.0 / (abs(gap - period) + 1.0)
    return np.argsort(scores)[::-1]

def generate_power_precision_3bet(history):
    # 注 1 & 2: Fourier Rhythm
    f_rank = get_fourier_rank(history)
    idx_1 = 0
    while idx_1 < len(f_rank) and f_rank[idx_1] == 0: idx_1 += 1
    bet1 = sorted(f_rank[idx_1:idx_1+6].tolist())

    idx_2 = idx_1 + 6
    while idx_2 < len(f_rank) and f_rank[idx_2] == 0: idx_2 += 1
    bet2 = sorted(f_rank[idx_2:idx_2+6].tolist())

    exclude = set(bet1) | set(bet2)

    # 注 3: Echo + Cold
    if len(history) >= 2:
        echo_nums = [n for n in history[-2]['numbers'] if n <= 38 and n not in exclude]
    else:
        echo_nums = []

    recent = history[-100:]
    freq = Counter([n for d in recent for n in d['numbers']])
    remaining = [n for n in range(1, 39) if n not in exclude and n not in echo_nums]
    remaining.sort(key=lambda x: freq.get(x, 0)) # Coldest first

    bet3 = sorted((echo_nums + remaining)[:6])

    return [bet1, bet2, bet3]

def main():
    db_path = os.path.join(project_root, 'lottery_api', 'data', 'lottery_v2.db')
    db = DatabaseManager(db_path)
    draws = sorted(db.get_all_draws('POWER_LOTTO'), key=lambda x: (x['date'], x['draw']))

    last_draw = draws[-1]
    next_draw = int(last_draw['draw']) + 1
    bets = generate_power_precision_3bet(draws)

    print("=" * 70)
    print(f"  威力彩 Power Precision 3注預測 — 第 {next_draw} 期")
    print("=" * 70)
    print(f"  策略: Power Precision (F2 + Echo/Cold)")
    print(f"  上期開獎: {last_draw['draw']} → {last_draw['numbers']}")
    print("=" * 70)
    print()

    for i, bet in enumerate(bets):
        label = ["Fourier 注1", "Fourier 注2", "Echo/Cold"][i]
        print(f"  注 {i+1}: [{', '.join(f'{n:02d}' for n in bet)}] ({label})")

    # 特別號
    try:
        from models.special_predictor import PowerLottoSpecialPredictor
        rules = {'name': 'POWER_LOTTO', 'specialMinNumber': 1, 'specialMaxNumber': 8}
        sp = PowerLottoSpecialPredictor(rules)
        sp_top = sp.predict_top_n(draws, n=1)
        print(f"\n  特別號 (V3 MAB): {sp_top[0]}")
    except: pass
    print("=" * 70)

if __name__ == "__main__":
    main()
