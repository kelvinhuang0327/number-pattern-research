...
class BiglottoGraph:
    ...
    def build_from_history(self, history, lookback=250):
        """
        从历史数据构建图 (每次构建前清理旧数据)
        期待 history 为 ASC 顺序 (旧到新)
        """
        self.graph.clear()
        self.node_features = {}
        self.edge_weights = {}
        ...
        # Get recent history from the END (ASC)
        recent_history = history[-lookback:] if len(history) > lookback else history
        
        print(f"🔨 构建号码关联图 (使用最近 {len(recent_history)} 期數據)")
    ...
    def get_node_feature_matrix(self):
        ...
        for num in range(self.min_num, self.max_num + 1):
            ...
            features.append([
                ...
                min(1.0, feat.get('recency', 0) / 30.0),  # Recent hit = 0, distant = 1 (capped at 1.0)
    ...
    def get_transition_probability_matrix(self):
        """
        获取行归一化的邻接矩阵 (用于马可夫转移或 GNN 传播)
        P = D^-1 * A
        """
        adj = self.get_adjacency_matrix()
        # Add self-loops to prevent dead ends and stabilize propagation
        adj = adj + np.eye(adj.shape[0], dtype=np.float32) * 0.1
    ...
    def analyze_graph_properties(self):
        ...
        return {
            ...
            'communities': [list(c) for c in communities] if 'communities' in locals() else []
        }
