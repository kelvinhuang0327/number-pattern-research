...
async def predict_biglotto_8_bets():
    print('=' * 80)
    print('🎯 大樂透最佳8注預測（下期預測）')
    print('=' * 80)
    print()
    ...
    # 使用最近200期作為訓練數據（集成預測需要至少151期）
    history = all_draws[:200]
    ...
    if len(bets) >= 8:
        ...
        print('=' * 80)
        print()
        print('✓ 8注號碼已按信心度由高到低排序')
        print('✓ 建議優先關注前4-5注（信心度較高）')
        print('✓ 超高頻號碼出現在多組預測中，為核心推薦號碼')
        print('✓ 8組號碼涵蓋不同預測策略，提高覆蓋率')
        print()
        print('📊 大樂透中獎機率說明:')
        print('  • 頭獎（6個號碼全中）: 1/13,983,816')
        print('  • 貳獎（5個號碼+特別號）: 1/2,330,636')
        print('  • 參獎（5個號碼）: 1/55,492')
        print('  • 投入金額: NT$ 800（8注×100元）')
        print()
        print('⚠️  重要提醒')
        print('   • 彩券為機率遊戲，無法保證中獎')
        print('   • 預測僅供參考，請理性投注')
        print('   • 大樂透中獎機率遠低於今彩539')
        ...
        print(f'💰 投注金額: NT$ {len(bets_sorted) * 100}')
        print('=' * 80)
...
if __name__ == '__main__':
    asyncio.run(predict_biglotto_8_bets())
