"""P541B-R2: fail-closed evidence audit for the pinned current-main corpus.

The historical P541B artifact supplies the governed seed paths and their
non-safety classification context.  The seed is expanded with every matching
current-main addition under the documented candidate roots, while every
matched exclusion is recorded explicitly.  Source discovery and reads use Git
objects from ``FROZEN_SOURCE_COMMIT`` only: no source module is imported or
executed, no mutable working-tree source is scanned, and no database is opened.
"""
...
DETECTOR_VERSION = "p541b-r2-detector-v10-fresh-main-r1"
SCOPE_VERSION = "p541b-r2-fresh-main-scope-v1"
CANONICAL_RUNTIME_IMPLEMENTATION = "CPython"
CANONICAL_RUNTIME_VERSION = "3.9.6"
BASE_MAIN_COMMIT = "e0f6c26f2c15d8b1e75ae1eb8df5b5aa97977a65"
FROZEN_SOURCE_COMMIT = BASE_MAIN_COMMIT
HISTORICAL_RECORD_COUNT = 580
GENERATED_AT_UTC = "2026-07-19T10:20:38Z"
...
BIG_LOTTO_SCOPE_MARKERS = (b"big_lotto", b"biglotto", "大樂透".encode("utf-8"))
CURRENT_MAIN_SCOPE_GROUPS = (
    {"name": "p541a_tools", "kind": "prefix", "value": "tools/"},
    {"name": "p541a_analysis", "kind": "prefix", "value": "analysis/"},
    {"name": "root_level_scripts", "kind": "root", "value": "*.py"},
    {"name": "ai_lab", "kind": "prefix", "value": "ai_lab/"},
    {
        "name": "recovered_strategies_biglotto",
        "kind": "prefix",
        "value": "recovered_strategies/biglotto/",
    },
    {
        "name": "lottery_api_models",
        "kind": "prefix",
        "value": "lottery_api/models/",
    },
    {
        "name": "lottery_api_tools",
        "kind": "prefix",
        "value": "lottery_api/tools/",
    },
    {
        "name": "lottery_api_engine",
        "kind": "prefix",
        "value": "lottery_api/engine/",
    },
)
EXPLICIT_DISCOVERY_EXCLUSIONS = {
    "lottery_api/models/replay_strategy_registry.py": "existing_registry_wiring",
    "lottery_api/models/p42_wave3_biglotto_adapters.py": "existing_adapter_wiring",
    "lottery_api/models/p93_tierb_replay_adapters.py": "existing_adapter_wiring",
    "tests/test_p263a_d3_strategy_status_coverage_audit.py": "test_source",
    "tools/quick_predict.py": "existing_dispatcher_wiring",
}
FRAMEWORK_FILE_BASENAMES = frozenset(
    {
        "strategy_adapter.py",
...
OUT_OF_SCOPE_PREFIX_REASONS = (
    ("tests/", "test_source"),
    ("scripts/", "operational_task_script"),
    ("lottery_api/routes/", "api_route_infrastructure"),
    ("lottery_api/utils/", "shared_utility_infrastructure"),
    ("lottery_api/fetcher/", "data_fetch_infrastructure"),
    ("lottery_api/diagnostics/", "diagnostic_infrastructure"),
)
...
def git_tree_entries(
    ...
    for record in raw.split(b"\x00"):
        ...
        if path in result:
            raise P541BR2Error(f"duplicate Git tree path: {path}")
        result[path] = {"mode": mode, "type": object_type, "blob_id": oid}
    return result


def git_recursive_tree_entries(
    repo_root: Path, commit: str
) -> dict[str, dict[str, str]]:
    """Return the complete pinned tree without traversing any gitlink."""
    raw = run_git(repo_root, ["ls-tree", "-r", "-z", commit])
    result: dict[str, dict[str, str]] = {}
    for record in raw.split(b"\x00"):
        if not record:
            continue
...
def validate_historical_payload(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if (
        ...
        or len(records) != HISTORICAL_RECORD_COUNT
    ):
...
def _scope_group_candidates(path: str) -> list[str]:
    if PurePosixPath(path).suffix != ".py":
        return []
    candidates: list[str] = []
    for rule in CURRENT_MAIN_SCOPE_GROUPS:
        if rule["kind"] == "root":
            matches = "/" not in path
        elif rule["kind"] == "prefix":
            matches = path.startswith(rule["value"])
        else:
            raise P541BR2Error(f"unknown scope rule kind: {rule['kind']}")
        if matches:
            candidates.append(rule["name"])
    return candidates


def _scope_exclusion_reason(path: str, group_candidates: Sequence[str]) -> str | None:
    explicit = EXPLICIT_DISCOVERY_EXCLUSIONS.get(path)
    if explicit is not None:
        return explicit
    if (
        path.startswith("lottery_api/models/")
        and PurePosixPath(path).name in FRAMEWORK_FILE_BASENAMES
    ):
        return "shared_model_framework"
    if group_candidates:
        return None
    for prefix, reason in OUT_OF_SCOPE_PREFIX_REASONS:
        if path.startswith(prefix):
            return reason
    return "outside_governed_candidate_roots"


def _matches_big_lotto_scope(raw: bytes) -> bool:
    lowered = raw.lower()
    return any(marker in lowered for marker in BIG_LOTTO_SCOPE_MARKERS)


def _fresh_main_addition_context(source_path: str, discovery_group: str) -> dict[str, Any]:
    source_type = {
        "p541a_tools": "tool_script",
        "p541a_analysis": "analysis_script",
        "root_level_scripts": "legacy_script",
        "ai_lab": "legacy_script",
        "recovered_strategies_biglotto": "legacy_script",
        "lottery_api_models": "adapter",
        "lottery_api_tools": "tool_script",
        "lottery_api_engine": "legacy_script",
    }.get(discovery_group)
    if source_type is None:
        raise P541BR2Error(f"unclassified discovery group: {discovery_group}")
    return {
        "normalized_name": PurePosixPath(source_path).stem,
        "discovery_group": discovery_group,
        "source_type": source_type,
        "method_family": "unknown",
        "is_actual_prediction_method": "unknown",
        "has_registry_entry": False,
        "has_adapter": False,
        "has_replay_rows": "unknown",
        "appears_in_artifacts": False,
        "duplicate_of_existing_strategy": None,
        "runnable_status": "ambiguous_needs_cto_review",
        "why_not_runnable": (
            "Current-main addition was not present in the historical P541B identity "
            "classification. Identity remains explicitly unknown and fail-closed pending "
            "a separately authorized review."
        ),
        "rewrite_needed": "unknown",
        "estimated_effort": "unknown",
        "recommended_action": "needs_cto_review",
        "confidence": "low",
        "legacy_script_group_category": "unknown",
    }


def discover_current_main_scope(
    repo_root: Path,
    ...
    """Build the deterministic historical-seed-plus-current-additions corpus."""
    historical_by_path: dict[str, dict[str, Any]] = {}
    for index, record in enumerate(historical_records):
        if not isinstance(record, dict):
            raise P541BR2Error(f"historical scope record is not an object: {index}")
        source_path = record.get("source_path")
        method_id = record.get("method_id")
        if not isinstance(source_path, str) or not isinstance(method_id, str):
            raise P541BR2Error(f"historical scope identity missing: {index}")
        _safe_repo_path(source_path)
        if source_path in historical_by_path:
            raise P541BR2Error(f"duplicate historical scope path: {source_path}")
        historical_by_path[source_path] = record

    tree_entries = git_recursive_tree_entries(repo_root, source_commit)
    missing = sorted(set(historical_by_path) - set(tree_entries))
    if missing:
        raise P541BR2Error(f"current-main historical seed missing: {missing[:5]}")

    scoped: dict[str, dict[str, Any]] = {}
    for source_path, record in historical_by_path.items():
        entry = tree_entries[source_path]
        if (
            PurePosixPath(source_path).suffix != ".py"
            or entry["type"] != "blob"
            or entry["mode"] not in {"100644", "100755"}
        ):
            raise P541BR2Error(
                f"current-main historical seed is not a regular Python blob: {source_path}"
            )
        discovery_group = record.get("discovery_group")
        known_groups = {rule["name"] for rule in CURRENT_MAIN_SCOPE_GROUPS}
        if discovery_group not in known_groups:
            raise P541BR2Error(f"historical discovery group missing: {source_path}")
        scoped[source_path] = {
            "method_id": record["method_id"],
            "source_path": source_path,
            "historical_context": _historical_context(record),
            "scope_membership": {
                "scope_version": SCOPE_VERSION,
                "origin": "historical_governed_seed",
                "discovery_group": discovery_group,
                "source_path": source_path,
            },
        }

    excluded_keyword_matches: list[dict[str, str]] = []
    current_main_additions: list[str] = []
    for source_path, entry in sorted(tree_entries.items()):
        if source_path in historical_by_path or PurePosixPath(source_path).suffix != ".py":
            continue
        group_candidates = _scope_group_candidates(source_path)
        if entry["type"] != "blob" or entry["mode"] not in {"100644", "100755"}:
            if group_candidates:
                raise P541BR2Error(
                    f"current-main candidate is not a regular Python blob: {source_path}"
                )
            continue
        raw = git_blob(repo_root, entry["blob_id"])
        if not _matches_big_lotto_scope(raw):
            continue
        if len(group_candidates) > 1:
            raise P541BR2Error(
                f"ambiguous current-main discovery group: {source_path}: {group_candidates}"
            )
        exclusion_reason = _scope_exclusion_reason(source_path, group_candidates)
        if exclusion_reason is not None:
            excluded_keyword_matches.append(
                {"source_path": source_path, "reason": exclusion_reason}
            )
            continue
        if len(group_candidates) != 1:
            raise P541BR2Error(f"unclassified current-main source identity: {source_path}")
        discovery_group = group_candidates[0]
        context = _fresh_main_addition_context(source_path, discovery_group)
        scoped[source_path] = {
            "method_id": source_path,
            "source_path": source_path,
            "historical_context": context,
            "scope_membership": {
                "scope_version": SCOPE_VERSION,
                "origin": "current_main_addition",
                "discovery_group": discovery_group,
                "source_path": source_path,
            },
        }
        current_main_additions.append(source_path)

    ordered_records = [scoped[path] for path in sorted(scoped)]
    memberships = [record["scope_membership"] for record in ordered_records]
    scoped_entries = {
        record["source_path"]: tree_entries[record["source_path"]]
        for record in ordered_records
    }
    scope_manifest = {
        "scope_version": SCOPE_VERSION,
        "source_commit": source_commit,
        "historical_seed_count": len(historical_by_path),
        "current_main_addition_count": len(current_main_additions),
        "record_count": len(ordered_records),
        "inclusion_policy": (
            "Retain every historical governed path, then add every regular current-main "
            "Python blob containing a BIG_LOTTO scope marker under exactly one governed "
            "candidate root unless an explicit exclusion rule applies."
        ),
        "scope_markers": ["big_lotto (case-insensitive)", "biglotto (case-insensitive)", "大樂透"],
        "governed_groups": list(CURRENT_MAIN_SCOPE_GROUPS),
        "exclusion_rules": {
            "exact_paths": [
                {"source_path": path, "reason": reason}
                for path, reason in sorted(EXPLICIT_DISCOVERY_EXCLUSIONS.items())
            ],
            "model_framework_basenames": sorted(FRAMEWORK_FILE_BASENAMES),
            "out_of_scope_prefixes": [
                {"prefix": prefix, "reason": reason}
                for prefix, reason in OUT_OF_SCOPE_PREFIX_REASONS
            ],
            "default": "outside_governed_candidate_roots",
            "historical_seed_precedence": (
                "An existing historical governed path remains included even if a current "
                "keyword or exclusion rule would no longer discover it."
            ),
        },
        "ordered_current_main_additions": sorted(current_main_additions),
        "excluded_keyword_matches": excluded_keyword_matches,
        "ordered_included_membership": memberships,
        "canonicalization": "UTF-8 compact sorted-key JSON in source-path order",
        "canonical_sha256": sha256_bytes(canonical_bytes(memberships)),
        "verification": "PASS",
    }
    return ordered_records, scoped_entries, scope_manifest
...
def require_current_main_entries(
    paths: Sequence[str], entries: dict[str, dict[str, str]]
) -> None:
    missing = [path for path in paths if path not in entries]
    if missing or len(entries) != len(paths) or len(set(paths)) != len(paths):
        raise P541BR2Error(f"current-main source corpus incomplete: {missing[:5]}")
...
def validate_artifact(artifact: dict[str, Any]) -> None:
    ...
    if (
        ...
        or artifact.get("current_main_source_commit") != FROZEN_SOURCE_COMMIT
    ):
    ...
    if not isinstance(records, list) or not records:
        raise P541BR2Error("artifact must contain current-main records")
    ...
    if (
        ...
        or any(not isinstance(method_id, str) or not method_id for method_id in method_ids)
        or len(set(paths)) != record_count
        or len(set(method_ids)) != record_count
    ...
    for record in records:
        ...
        membership = record.get("scope_membership", {})
        if (
            membership.get("scope_version") != SCOPE_VERSION
            or membership.get("source_path") != record.get("source_path")
            or membership.get("origin")
            not in {"historical_governed_seed", "current_main_addition"}
            or not isinstance(membership.get("discovery_group"), str)
            or not membership["discovery_group"]
        ):
            raise P541BR2Error(
                f"scope membership mismatch: {record.get('source_path')}"
            )
    ...
    scope_manifest = artifact.get("provenance", {}).get("fresh_main_scope", {})
    memberships = [record["scope_membership"] for record in records]
    current_main_additions = [
        membership["source_path"]
        for membership in memberships
        if membership["origin"] == "current_main_addition"
    ]
    historical_seed_count = sum(
        membership["origin"] == "historical_governed_seed"
        for membership in memberships
    )
    excluded_matches = scope_manifest.get("excluded_keyword_matches")
    if (
        scope_manifest.get("scope_version") != SCOPE_VERSION
        or scope_manifest.get("source_commit") != FROZEN_SOURCE_COMMIT
        or scope_manifest.get("historical_seed_count") != historical_seed_count
        or scope_manifest.get("current_main_addition_count")
        != len(current_main_additions)
        or scope_manifest.get("record_count") != record_count
        or scope_manifest.get("ordered_current_main_additions")
        != current_main_additions
        or scope_manifest.get("ordered_included_membership") != memberships
        or scope_manifest.get("canonical_sha256")
        != sha256_bytes(canonical_bytes(memberships))
        or scope_manifest.get("governed_groups") != list(CURRENT_MAIN_SCOPE_GROUPS)
        or scope_manifest.get("verification") != "PASS"
        or not isinstance(excluded_matches, list)
        or any(
        ...
        or [item["source_path"] for item in excluded_matches]
        != sorted(item["source_path"] for item in excluded_matches)
        or set(paths).intersection(item["source_path"] for item in excluded_matches)
    ):
        raise P541BR2Error("fresh-main scope manifest invariant mismatch")
    summary = artifact.get("summary", {})
    scan_counts = summary.get("scan_status_counts", {})
    if (
        summary.get("total_records") != record_count
        or list(scan_counts) != list(SCAN_STATUS_TAXONOMY)
        or sum(scan_counts.values()) != record_count
        or summary.get("complete_scans") != scan_counts["complete"]
        or summary.get("unknown_scans") != record_count - scan_counts["complete"]
    ):
    ...
    for key in ALL_EVIDENCE_KEYS:
        ...
        if (
            ...
            or sum(published.values()) != record_count
...
def build_artifact(repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    scope_records, entries, scope_manifest = discover_current_main_scope(
        repo_root,
        historical_records,
        FROZEN_SOURCE_COMMIT,
    )
    paths = [record["source_path"] for record in scope_records]
    require_current_main_entries(paths, entries)
    ...
    for scope_record in scope_records:
        source_path = scope_record["source_path"]
        _safe_repo_path(source_path)
        ...
        if (
            ...
            raise P541BR2Error(
                f"current-main source is not a regular Python blob: {source_path}"
            )
        try:
            raw = git_blob(repo_root, entry["blob_id"])
        except GitUnavailableError:
            ...
            records.append(
                {
                    "method_id": scope_record["method_id"],
                    "source_path": source_path,
                    **analysis,
                    "historical_p541b_classification": scope_record[
                        "historical_context"
                    ],
                    "scope_membership": scope_record["scope_membership"],
                }
            )
            continue
    ...
    artifact = {
        ...
        "current_main_source_commit": FROZEN_SOURCE_COMMIT,
        "generator": {
            "path": GENERATOR_PATH,
        ...
        "detector_contract": {
            ...
            "scope_version": SCOPE_VERSION,
            "scope_policy": scope_manifest["inclusion_policy"],
            "one_hop_policy": (
        ...
        "provenance": {
            ...
            "fresh_main_scope": scope_manifest,
            "source_manifest": {
                "record_count": len(records),
                "entry_fields": [
...
def render_markdown(artifact: dict[str, Any]) -> str:
    ...
    scope = artifact["provenance"]["fresh_main_scope"]
    lines = [
        "# P541B R2 — Fail-Closed Structured Evidence Classification",
        "",
        f"> generated_at_utc: `{artifact['generated_at_utc']}`",
        "",
        "## Scope and Pinned Current-Main Corpus",
        "",
        f"- Implementation base: `{artifact['implementation_base_oid']}`",
        f"- Pinned current-main source commit: `{artifact['current_main_source_commit']}`",
        f"- Scope contract: `{scope['scope_version']}` — verification **{scope['verification']}**.",
        f"- Ordered corpus: **{scope['record_count']}** unique method IDs and source paths "
        f"({scope['historical_seed_count']} historical governed seeds + "
        f"{scope['current_main_addition_count']} current-main additions).",
        "- Historical governed paths are retained; matching current-main additions are "
        "discovered from the pinned Git tree and every keyword-match exclusion is recorded.",
        "- Source bytes were read only with Git plumbing from the pinned current-main commit.",
        ...
        "- Repository/Git unavailability, base/scope-manifest failure, duplicates, top-level invariants, and serialization failures remain terminal.",
        ...
        "- Detector-v10/R9 successor traversal seals dormant nested scopes, evaluates eager decorators/defaults/class bodies, bounds sibling/mutual/self recursion, resolves aliases/factories/callbacks/bound methods, propagates incomplete resolution, and fails closed for escaped nested callables.",
        "",
        "## Fresh-Main Scope Drift",
        "",
        f"- Scope-manifest SHA-256: `{scope['canonical_sha256']}`",
        f"- Current-main additions ({scope['current_main_addition_count']}):",
    ]
    lines.extend(
        f"  - `{path}`" for path in scope["ordered_current_main_additions"]
    )
    lines.append(
        f"- Excluded current-main keyword matches ({len(scope['excluded_keyword_matches'])}):"
    )
    lines.extend(
        f"  - `{item['source_path']}` — `{item['reason']}`"
        for item in scope["excluded_keyword_matches"]
    )
    lines.extend([
        "",
        "## Summary",
        ...
        "| Metric | Value |",
        "|---|---:|",
        f"| Governed current-main source records | {summary['total_records']} |",
    ...
    lines.extend([
        ...
        f"- Current-main source manifest: {artifact['provenance']['source_manifest']['record_count']} Git blobs, canonical SHA-256 `{artifact['provenance']['source_manifest']['canonical_sha256']}` — verification **PASS**",
        f"- Recovered source blob-read failures: **{artifact['provenance']['source_manifest']['content_read_failures']}**",
        "- Discovery uses the pinned Git tree only; mutable working-tree source discovery is prohibited.",
        "",
        "## Downstream Requirement",
        "",
        "The authorized P541C successor must consume this exact artifact identity, regenerate every derived count and shortlist decision, and must never coerce `unknown` to `false`.",
        "",
        "## Limitations",
        "",
    ])
