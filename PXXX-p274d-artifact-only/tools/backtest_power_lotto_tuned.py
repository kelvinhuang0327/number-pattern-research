...
from ai_lab.scripts.power_lotto_tuned_anomaly import PowerLottoOptimized2Bet
...
def main():
    ...
    predictor = PowerLottoOptimized2Bet(engine)
    
    print("=" * 80)
    print("🔬 Phase 35: Power Lotto Optimized 2-Bet (Statistical + Tuned Anomaly)")
    print("=" * 80)
    ...
    wins = 0
    match_dist = Counter()
    bet_wins = {'Statistical': 0, 'Anomaly': 0}
    zone2_wins = 0
    ...
    z2_rate = zone2_wins / (total * 2) * 100 if total > 0 else 0
    
    print("-" * 80)
    print(f"✅ Final Rate (150期): {rate:.2f}%")
    ...
    print(f"🏆 Bet Contribution: Statistical: {bet_wins['Statistical']}, Anomaly: {bet_wins['Anomaly']}")
    print("=" * 80)
    print(f"\n📊 Comparison:")
    print(f"   Statistical + Tuned Anomaly: {rate:.2f}%")
