...
from orchestrator import db, common, execution_policy, health, planner_guard, failure_taxonomy, repair_task_generator, outcome_extractor, llm_audit
from orchestrator.provider_audit_guard import (
    is_external_provider as _is_ext_worker_prov,
    write_worker_result_by_task_id as _write_worker_audit_result,
)
...
def _finalize(lock: dict):
    ...
    # ── Audit: write LLM_CALL_RESULT closing the open ATTEMPT for this task ──
    _is_rate_limit = (final_status == "FAILED_RATE_LIMIT")
    _audit_result_prov = meta.get("worker_dispatch_provider") or meta.get("worker_provider") or meta.get("worker_runtime") or ""
    if _is_ext_worker_prov(_audit_result_prov):
        _write_worker_audit_result(
            task_id=task_id,
            ...
            runner_type="worker",
            usage_role="worker",
            provider=_audit_result_prov,
            model=meta.get("worker_model") or "",
            trigger_source="worker_claim",
            rate_limit_type="Requests" if _is_rate_limit else None,
        )
    ...
    try:
        db.backfill_llm_usage_metrics_for_task(
            ...
            parse_source="worker_finalize_stdout",
            usage_role="worker",
            runner_type="worker",
        )
    except Exception as _bfe:
        logger.debug("Worker backfill_llm_usage error (non-fatal): %s", _bfe)
...
def _claim(task: dict):
    ...
    if not launch_decision.allowed:
        ...
        if _is_ext_worker_prov(dispatch_provider):
            llm_audit.write_blocked(
                runner_type="worker", usage_role="worker",
                provider=dispatch_provider, model=worker_model,
                block_reason=launch_decision.skip_reason or "execution_policy_blocked",
                task_id=task_id,
                trigger_source="worker_claim",
                extra_skip=1,
            )
    ...
    if _is_ext_worker_prov(dispatch_provider):
        ...
        _cap = check_llm_cap(
            usage_role="worker", runner_type="worker",
            provider=dispatch_provider, task_id=task_id,
            model=worker_model, trigger_source="worker_claim",
        )
        if not _cap.allowed:
            llm_audit.write_blocked(
                runner_type="worker", usage_role="worker",
                provider=dispatch_provider, model=worker_model,
                block_reason=_cap.block_reason,
                task_id=task_id,
                trigger_source="worker_claim",
                extra_skip=1,
            )
            db.log_tick(RUNNER, "WORKER_SKIP_LLM_CAP", task_id=task_id, message=_cap.block_reason)
            common.log_jsonl(RUNNER, "WORKER_SKIP_LLM_CAP", task_id=task_id, reason=_cap.block_reason)
            return False

    # ── Audit: write attempt before subprocess launch (fail-closed) ──
    if _is_ext_worker_prov(dispatch_provider):
        _audit_ok, _audit_corr_id = llm_audit.write_attempt(
            runner_type="worker", usage_role="worker",
            provider=dispatch_provider, model=worker_model,
            task_id=task_id,
            trigger_source="worker_claim",
        ...
        if not _audit_ok:
            llm_audit.write_audit_unavailable(
                runner_type="worker", usage_role="worker",
                provider=dispatch_provider, trigger_source="worker_claim",
            )
            db.log_tick(RUNNER, "WORKER_SKIP_AUDIT_UNAVAILABLE", task_id=task_id,
    ...
    # Write worker LLM attribution to DB
    _wa_label = common.normalize_llm_agent_label(dispatch_provider, worker_model, runner_type="worker")["display_label"]
    db.update_task(task_id, worker_agent_label=_wa_label, llm_call_count=1)
