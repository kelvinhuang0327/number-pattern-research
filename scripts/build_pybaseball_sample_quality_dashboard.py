#!/usr/bin/env python3
"""Build the P215-A pybaseball historical sample quality dashboard from P214-A artifacts only."""
...
SOURCE_MD = REPORT_DIR / "p214a_pybaseball_historical_sample_smoke.md"
SOURCE_JSON = REPORT_DIR / "p214a_pybaseball_historical_sample_smoke.json"
SOURCE_CSV = REPORT_DIR / "p214a_pybaseball_historical_sample_smoke.csv"
OUT_HTML = REPORT_DIR / "p215a_pybaseball_sample_quality_dashboard.html"
OUT_JSON = REPORT_DIR / "p215a_pybaseball_sample_quality_dashboard.json"
DISCLAIMER = "Historical pybaseball sample quality dashboard only. Not live predictions, not betting advice."
TASK_NAME = "P215-A pybaseball Historical Sample Quality Dashboard"
SUCCESS_BANNER = "P215-A PYBASEBALL SAMPLE QUALITY DASHBOARD PASS"
FAILURE_BANNER = "P215-A PYBASEBALL SAMPLE QUALITY DASHBOARD FAIL"
PREVIEW_ROW_LIMIT = 5
...
PROHIBITED_CLAIMS = (
    ...
    "No wagering recommendation claim.",
    "No deployable-system claim.",
    "No season-wide representativeness claim.",
)
...
def build_payload() -> dict[str, Any]:
    ...
    limitations = [
        "Dashboard metrics are computed from the fixed P214-A CSV snapshot only, not from a refreshed upstream pull.",
        "This dashboard reflects a small historical sample and should not be generalized beyond the bounded artifact.",
    ...
    return {
        "task": TASK_NAME,
        "status": "PASS_P214A_ARTIFACT_ONLY_SAMPLE_QUALITY_DASHBOARD",
        "disclaimer": DISCLAIMER,
        "historical_only_disclaimer": DISCLAIMER,
        "source_artifacts": source_artifacts,
        "source_hashes": source_hashes,
        "source_summary": {
            "p214_task": source_json_payload.get("task"),
            "p214_status": source_json_payload.get("status"),
            "p214_disclaimer": source_json_payload.get("disclaimer"),
            "p214_request": source_json_payload.get("request"),
            "p214_result_summary": source_json_payload.get("result_summary"),
            "p214_markdown_mentions_disclaimer": source_json_payload.get("disclaimer") in source_md_text,
        },
        "row_count": len(rows),
...
def _render_summary_list(payload: dict[str, Any]) -> str:
    source_result = payload["source_summary"]["p214_result_summary"] or {}
    items = [
        ("Status", payload["status"]),
        ("Dashboard row count", payload["row_count"]),
        ("Dashboard column count", payload["column_count"]),
        ("Source fetched row count", source_result.get("fetched_row_count")),
        ("Source snapshot row count", source_result.get("snapshot_row_count")),
    ]
    return "\n".join(
