...
if not hasattr(qp, 'resolve_predraw_ledger_path'):
    def _resolve_predraw_ledger_path(args):
        cli_path = getattr(args, 'predraw_ledger_path', None)
        if cli_path:
            return cli_path
        return os.environ.get('LOTTERY_PREDRAW_LEDGER_PATH') or None

    qp.resolve_predraw_ledger_path = _resolve_predraw_ledger_path

if not hasattr(qp, 'write_predraw_ledger_for_prediction'):
    ...
    def _compute_next_scheduled_draw_date(last_date_str, lottery_type, schedule_config):
    ...
    def _write_predraw_ledger_for_prediction(lottery_type, bets, num_bets, history, run_id, ledger_path,
                                            generation_source='tools/quick_predict.py'):
        if not history:
            print(f'  [predraw-ledger] {lottery_type}: no history available, skipped')
            return
        try:
            max_source_draw = pl.compute_max_source_draw(qp.DB_PATH, lottery_type)
            if max_source_draw is None:
                print(f'  [predraw-ledger] {lottery_type}: no source draws found in DB, skipped')
                return

            target_draw = int(qp.get_next_draw_number(history))
            last_date = history[-1].get('date')
            schedule_config = pl.load_schedule_config()
            target_draw_date = _compute_next_scheduled_draw_date(last_date, lottery_type, schedule_config)
    ...
    qp.write_predraw_ledger_for_prediction = _write_predraw_ledger_for_prediction
