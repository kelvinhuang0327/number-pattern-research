...
print("="*80)
print("🚀 Phase 1 優化效果驗證 - 快速回測（跳過耗時方法）")
print("="*80)
print()
...
print("🔍 測試方法:")
for i, (method, desc) in enumerate(core_methods, 1):
    print(f"  {i}. {desc}")
print()
...
for i, (method, desc) in enumerate(core_methods, 1):
    print(f"[{i}/{len(core_methods)}] 測試 {method}...", end=' ', flush=True)
...
if comparison:
    ...
    output_file = f'backtest_report_quick_{datetime.now().strftime("%Y%m%d_%H%M%S")}.md'
    ...
    for rank, result in enumerate(sorted_comparison, 1):
        ...
        # 標記已優化的方法
        marker = "✨" if 'frequency' in method.lower() or 'entropy' in method.lower() else "  "
