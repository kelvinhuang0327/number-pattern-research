...
try:
    ...
    PatternAwareTransformerPredictor = None
    logger.warning("PatternAwareTransformerPredictor disabled (torch missing)")

try:
    from .mab_ensemble import MABEnsemblePredictor
except ImportError:
    MABEnsemblePredictor = None
    logger.warning("MABEnsemblePredictor disabled (missing dependencies)")
try:
    from ..config_loader import PredictionConfig
...
def predict_pool_numbers(
    ...
    # 執行策略
    # Use global singleton to avoid re-initialization overhead
...
class UnifiedPredictionEngine:
    ...
    def __init__(self):
        ...
        # Phase 54: Meta-Stacking Stacker
        strategy_names = [
        ...
        if CONFIG_AVAILABLE:
            ...
            if self.config.get('experimental.mab_enabled', False) and MABEnsemblePredictor is not None:
                mab_config = self.config.get('experimental.mab_config', {})
                # Prepare predictors map for MAB
                self.strategies_map = {
                ...
                self.engine_mab = MABEnsemblePredictor(self.strategies_map, mab_config)
                self.mab_predictor = self.engine_mab
                logger.info("Initialized MABEnsemblePredictor")
            else:
                self.mab_predictor = None
        else:
            self.mab_predictor = None
            
        # Phase 42-44: Regime Detection & Zonal Pruning
        try:
            from .lag_reversion import LagReversionPredictor
            from .zone_cluster import ZoneClusterRefiner
            from .regime_detector import RegimeDetector
            from .vae_predictor import VAEPredictor
            self.lag_predictor = LagReversionPredictor({})
            self.zone_refiner = ZoneClusterRefiner({})
            self.regime_detector = RegimeDetector()
            self.vae_model = VAEPredictor()
        except ImportError:
            self.lag_predictor = None
            self.zone_refiner = None
            self.regime_detector = None
            self.vae_model = None

        self._wrap_strategies()
        logger.info("UnifiedPredictionEngine 初始化完成 (優化版 + 策略緩存)")

    def auto_identify_stability(self, lottery_type: str, strategy_name: str, result: Dict) -> Dict:
        """
        自動識別策略穩定性並附加到預測結果中。
        """
        profile_mgr = get_stability_profile()
        stability = profile_mgr.get_strategy_stability(lottery_type, strategy_name)
        
        result['stability_metrics'] = {
            'status': stability['status'],
            'description': stability['description'],
            'history': stability['data']
        }
        
        # 額外警報
        if stability['status'] == 'SHORT_MOMENTUM':
            logger.info(f"⚠️ [STABILITY ALERT] {strategy_name} for {lottery_type} has high decay in long-term benchmarks.")
    ...
    def _make_cached(self, func, name):
        def wrapper(history, lottery_rules, *args, **kwargs):
            ...
            if cache_key in self._shared_prediction_cache:
                return self._shared_prediction_cache[cache_key]
                
            res = func(history, lottery_rules, *args, **kwargs)
    ...
    def _wrap_strategies(self):
        strategy_names = [
            'statistical_predict', 'deviation_predict', 'trend_predict',
            'frequency_predict', 'arima_predict', 'smart_markov_predict',
            'bayesian_predict', 'monte_carlo_predict', 'markov_predict',
            'hot_cold_mix_predict', 'anomaly_detection_predict',
            'zone_balance_predict', 'sum_range_predict',
            'odd_even_balance_predict', 'ai_lstm_predict',
            'gap_sensitive_predict', 'extended_sum_range_predict',
        ...
        for name in strategy_names:
            if hasattr(self, name):
                orig_method = getattr(self, name)
                # 我們不使用裝飾器，直接替換實例方法
                setattr(self, name, self._make_cached(orig_method, name))

    def _get_strategy_config(self, lottery_name: str, strategy_name: str, config_key: str, default: Any) -> Any:
        """獲取特定彩種的策略配置"""
        if not hasattr(self, 'config'):
            return default
            
        # 1. 嘗試獲取彩種特定配置: strategies.BIG_LOTTO.trend.lambda
        if lottery_name:
            # 轉換為標準名稱 (BIG_LOTTO, POWER_LOTTO)
            canonical_name = 'BIG_LOTTO' if 'BIG_LOTTO' in lottery_name or '大樂透' in lottery_name else \
                             'POWER_LOTTO' if 'POWER_LOTTO' in lottery_name or '威力彩' in lottery_name else \
                             lottery_name
            
            val = self.config.get(f'strategies.{canonical_name}.{strategy_name}.{config_key}')
            if val is not None:
                return val
                
        # 2. 退回到全域配置: strategies.trend.lambda
        return self.config.get(f'strategies.{strategy_name}.{config_key}', default)
    ...
    def trend_predict(
        ...
        # Get parameters from config or rules
        lambda_decay = lottery_rules.get('trend_lambda')
        if lambda_decay is None:
            lambda_decay = self._get_strategy_config(lottery_rules.get('name'), 'trend', 'lambda', 0.05)
        ...
        is_asc = history[0].get('date', '0') < history[-1].get('date', '9')
        
        # 我們只分析最近 100 期以提高效率與相關性
        analysis_window = 100
        
        if is_asc:
            # ASC (Old -> New): 取最後 100 期 (Newest)
            recent_history = history[-analysis_window:]
            # 遍歷時，我們希望 Latest 的 age=0
            # recent_history: [Old, ..., New]
            # reversed: [New, ..., Old]
            # enumerate i=0 (New) -> age=0. Correct.
            iterable_history = reversed(recent_history)
        else:
            # DESC (New -> Old): 取前 100 期 (Newest)
            recent_history = history[:analysis_window]
            # recent_history: [New, ..., Old]
            # enumerate i=0 (New) -> age=0. Correct.
            iterable_history = recent_history

        for i, draw in enumerate(iterable_history):
            age = i
    ...
    def deviation_predict(
        ...
        # Get weights from config or rules
        default_weights = {
            'frequency': 0.30,
            'zone': 0.25,
            'odd_even': 0.20,
        ...
        weights = lottery_rules.get('deviation_weights')
        if not weights:
            weights = self._get_strategy_config(lottery_rules.get('name'), 'deviation', 'weights', default_weights)
        ...
        scores = freq_scores * weights.get('frequency', 0.30)  # 權重30%
        ...
        for zone_id, score in zone_scores.items():
            for num in zones[zone_id]:
                if min_num <= num <= max_num:
                    scores[num] += score * weights.get('zone', 0.25) / len(zones[zone_id])
        ...
        for i in range(min_num, max_num + 1):
            if i % 2 == 1:  # 奇數
                if odd_deviation > 0:  # 奇數太少，需要補償
                    scores[i] += weights.get('odd_even', 0.20) * (odd_deviation / expected_odd)
            else:  # 偶數
                if odd_deviation < 0:  # 偶數太少，需要補償
                    scores[i] += weights.get('odd_even', 0.20) * (abs(odd_deviation) / expected_odd)
        ...
        for i in range(min_num, max_num + 1):
            if i <= mid_point:  # 小號
                if small_deviation > 0:
                    scores[i] += weights.get('high_low', 0.15) * (small_deviation / expected_small)
            else:  # 大號
                if small_deviation < 0:
                    scores[i] += weights.get('high_low', 0.15) * (abs(small_deviation) / expected_small)
        ...
        for num in range(min_num, max_num + 1):
            ...
            scores[num] += gap_score * weights.get('gap', 0.10)
    ...
    def frequency_predict(
        ...
        # 從最新的數據開始遍歷 (Fix History Order)
        if not history:
            return {'numbers': []}
            
        is_asc = history[0].get('date', '0') < history[-1].get('date', '9')
        analysis_window = 200 # 頻率分析看長一點
        
        if is_asc:
            # ASC: [Old ... New] -> 取 [-200:]
            recent_history = history[-analysis_window:]
            # 我们需要 i=0 對應 Latest (Newest)
            # reversed([Old ... New]) -> [New ... Old]. i=0 is New. Correct.
            iterable_history = reversed(recent_history)
        else:
            # DESC: [New ... Old] -> 取 [:200]
            recent_history = history[:analysis_window]
            # [New ... Old]. i=0 is New. Correct.
            iterable_history = recent_history

        for i, draw in enumerate(iterable_history):
            # i 是距離現在的期數 (0=Latest, 1, 2...)
    ...
    def true_frequency_predict(
        self,
        history: List[Dict],
        lottery_rules: Dict
    ) -> Dict:
        """
        真實頻率策略 (True Frequency)
        純粹計算指定窗口內的出現次數，不進行衰減，用於捕捉 'Secondary Hot' (次熱門) 號碼
        預設窗口: 50期 (經回測驗證對捕捉 PowerLotto 熱號最有效)
        """
        pick_count = lottery_rules.get('pickCount', 6)
        min_num = lottery_rules.get('minNumber', 1)
        max_num = lottery_rules.get('maxNumber', 49)
        ...
        # 1. 確定分析窗口 (預設 50)
        window = lottery_rules.get('frequency_window', 50)
        
        # history 通常是 Old -> New 還是 New -> Old? 
        # 這裡需要小心處理。
        # 根據 frequency_predict 的邏輯:
        # is_asc = history[0] < history[-1] (Old -> New)
        # 我們需要最近的 50 期
        ...
        is_asc = history[0].get('date', '0') < history[-1].get('date', '9')
        if is_asc:
            target_history = history[-window:]
        else:
            target_history = history[:window]
            
        # 2. 統計次數
        all_nums = []
        for d in target_history:
        ...
        # 3. 排序 (次數高 -> 低, 號碼 小 -> 大)
        # key: (-count, num)
        ranked_items = sorted(counts.items(), key=lambda x: (-x[1], x[0]))
        
        # 4. 選取
        # 注意: 這裡我們可能需要返回超過 pick_count 的數量供上層邏輯篩選 (如 Top 12)
        # 但標準接口只返回 pick_count 個
        # 我們將完整列表放入 meta_info
        
        top_nums = [x[0] for x in ranked_items[:pick_count]]
        
        # 🧪 套用全局特徵過濾 (僅針對 POWER_LOTTO)
        top_nums = self.filter_by_global_constraints(history, top_nums, lottery_rules)
        
        # 計算信心度 (基於 Top 號碼的頻率強度)
        if ranked_items:
            max_freq = ranked_items[0][1]
            avg_freq = len(all_nums) / (max_num - min_num + 1)
            confidence = min(0.9, 0.6 + (max_freq / avg_freq - 1) * 0.1)
        else:
            confidence = 0.5

        return {
            'numbers': sorted(top_nums),
            'confidence': float(confidence),
            'method': f'真實頻率 (近{window}期)',
            'probabilities': [], # 真實頻率不產生機率分佈，或可轉換為 normalized counts
            'counts': dict(ranked_items),
            'ranked_list': [x[0] for x in ranked_items] # 方便調用者獲取完整排名
        }
    ...
    def arima_predict(
        ...
        """
        ARIMA 趨勢預測策略 (p=1, d=0, q=0) - 經過網格搜索優化
        """
        from .arima_predictor import arima_predict
        
        # 🔧 記錄數據範圍
        log_data_range('ARIMA 趨勢分析', history)
        
        # 獲取參數 (可從配置中讀取)
        p = self._get_strategy_config(lottery_rules.get('name'), 'arima', 'p', 1)
        d = self._get_strategy_config(lottery_rules.get('name'), 'arima', 'd', 0)
        q = self._get_strategy_config(lottery_rules.get('name'), 'arima', 'q', 0)
        
        result = arima_predict(history, lottery_rules, p=p, d=d, q=q)
        ...
        return {
            'numbers': selected,
            'confidence': 0.138, # 基於 500 期回測結果
            'method': '連莊策略 (Repeater)',
            'ranked_list': last_nums
        }

    def smart_markov_predict(
        self,
        history: List[Dict],
        lottery_rules: Dict
    ) -> Dict:
        """
        智能馬可夫策略 (Smart Markov Strategy)
        針對 Power Lotto 特別號優化：結合「高連莊率」與「強勢轉移」
        邏輯：
        1. 分析近期 (500期) 轉移矩陣
        2. 取得上一期號碼 (Prev)
        3. 比較 P(Prev->Prev) 與 max(P(Prev->Any))
        4. 若 P(Repeat) 高或差距不大 -> 賭連莊
        5. 若 P(Transfer) 顯著高於 P(Repeat) -> 賭轉移
        """
        pick_count = lottery_rules.get('pickCount', 1)
        
        if not history:
             return {'numbers': [], 'confidence': 0.1, 'method': 'Smart Markov (No Data)'}
             
        # Normalize history (Old -> New)
        if history[0].get('date', '0') > history[-1].get('date', '9'):
            history = list(reversed(history))

        # Recent history for matrix (use last 500 or all available)
        recent_hist = history[-500:] if len(history) > 500 else history
        
        # Build Transition Matrix
        # Note: Input history here might be raw draws (with 'special' field) or processed lists
        # We need to handle both cases efficiently.
        
        # Check data format
        sample = recent_hist[0]
        if 'special' in sample:
            vals = [d['special'] for d in recent_hist if d.get('special')]
        elif 'numbers' in sample:
            # If 'numbers' holds the special number (processed by caller)
            vals = [d['numbers'][0] for d in recent_hist if d['numbers']]
        else:
            return {'numbers': [1], 'confidence': 0.1, 'method': 'Error'}
            
        if len(vals) < 2:
             return {'numbers': [1], 'confidence': 0.1, 'method': 'Insufficient Data'}
        ...
        # Analyze row for last_val
        row_counts = matrix[last_val]
        total_row = row_counts.sum()
        
        if total_row == 0:
            # No history for this number? Repeater default.
            return {'numbers': [last_val], 'confidence': 0.125, 'method': 'Smart Markov (Fallback Repeater)'}
            
        probs = row_counts / total_row
        
        # P(Repeat)
        p_repeat = probs[last_val]
        
        # Best Transition
        best_next = np.argmax(probs) # Index 0 is unused, argmax will handle 1-8
        p_best = probs[best_next]
        
        # Decision Logic (Based on 1876-draw analysis)
        # If Best Transition is significantly better than Repeat (> 3% difference?)
        # Or if Repeat is simply low (< 11%)?
        
        # Using a simple threshold: if Best > Repeat + 0.02 (2%), switch.
        # Otherwise stick to Repeater (simpler model preference).
        
        if best_next != last_val and p_best > (p_repeat + 0.02):
            selected = int(best_next)
            method = f"Smart Markov (Transition {last_val}->{selected})"
            conf = p_best
        else:
            selected = int(last_val)
            method = f"Smart Markov (Repeater {last_val})"
            conf = p_repeat
    ...
    def bayesian_predict(
        ...
        result = {
            ...
            'probabilities_all': {int(num): float(prob) for num, prob in posterior_probs.items()}
        }
    ...
    def markov_predict(
        ...
        # Standardize History Order (Old -> New)
        if history and len(history) > 1:
            if history[0].get('draw', '9') > history[-1].get('draw', '0'):
                 history = list(reversed(history))
    ...
    def markov_v2_predict(self, history: List[Dict], lottery_rules: Dict) -> Dict:
        """
        馬可夫鏈 V2 (Cross-Order Resonance / 跨階共鳴)
        
        核心原理：
        1. 同時計算 1、2、3 階轉移矩陣
        2. 計算各號碼在不同階數下的「共鳴分數」
        3. 只選擇在多個時間窗口（階數）下都具備高轉移概率的號碼
        """
        pick_count = lottery_rules.get('pickCount', 6)
        min_num = lottery_rules.get('minNumber', 1)
        max_num = lottery_rules.get('maxNumber', 49)
        ...
        if len(history) < 20:
            return self.markov_predict(history, lottery_rules)

        # 1. 計算各階轉移概率
        p1 = self._markov_order1(history, max_num, pick_count)
        p2 = self._markov_order2(history, max_num, pick_count)
        p3 = self._markov_order3(history, max_num, pick_count)

        # 2. 計算共鳴分數 (加權權重：3階 > 2階 > 1階)
        resonance_scores = np.zeros(max_num + 1)
        for i in range(min_num, max_num + 1):
            # 權重分配：3階(0.5), 2階(0.3), 1階(0.2)
            # 使用加權和而非乘積，以確保數值穩定性
            resonance_scores[i] = (p1[i] * 0.2) + (p2[i] * 0.3) + (p3[i] * 0.5)

        # 3. 排除最近一期號碼（連莊率由 Repeat Booster 處理，Markov V2 專注於轉移）
        last_numbers = history[-1]['numbers']
        for num in last_numbers:
            if num <= max_num:
                resonance_scores[num] *= 0.5

        # 4. 排序選號
        sorted_indices = np.argsort(resonance_scores)[::-1]
        predicted_numbers = []
        for idx in sorted_indices:
            if min_num <= idx <= max_num:
                predicted_numbers.append(int(idx))
        ...
        return {
            ...
            'method': '馬可夫鏈 V2 (跨階共鳴)',
            'resonance_stats': {
                'top_p1': sorted(p1.argsort()[-5:]),
                'top_p2': sorted(p2.argsort()[-5:]),
                'top_p3': sorted(p3.argsort()[-5:])
            }
        }
    ...
    def statistical_predict(self, history, lottery_rules):
        ...
        default_params = {
            ...
            'ac_min_mult': 0.15,
            'ac_max_mult': 0.35,
            'odd_tolerance': pick_count // 3 + 1,
            'spread_mult': 0.4,
            'unique_last_digits_min': max(3, int(pick_count * 0.6)),
            'weight_power': 0.5  # sqrt
        }
        
        params = lottery_rules.get('statistical_params')
        if not params:
            params = self._get_strategy_config(lottery_rules.get('name'), 'statistical', 'params', default_params)
        ...
        def check_conditions(numbers):
            ...
            sum_range = (theoretical_max - theoretical_min) * params.get('sum_range_mult', 0.6)
            ...
            min_ac = max(pick_count - 1, int(total_numbers * params.get('ac_min_mult', 0.15)))
            max_ac = min(pick_count * (pick_count - 1) / 2, int(total_numbers * params.get('ac_max_mult', 0.35)))
            if not (min_ac <= ac <= max_ac): return False
            
            # 奇偶比
            odd = sum(1 for n in numbers if n % 2 == 1)
            ideal_odd = round(pick_count / 2)
            if abs(odd - ideal_odd) > params.get('odd_tolerance', 2): return False
            
            # 極差
            spread = max(numbers) - min(numbers)
            min_spread = int(total_numbers * params.get('spread_mult', 0.4))
            if spread < min_spread: return False
            
            # 尾數多樣性
            last_digits = set(n % 10 for n in numbers)
            min_unique = params.get('unique_last_digits_min', 4)
        ...
        # 構建加權池
        pool = []
        power = params.get('weight_power', 0.5)
        for i in range(min_num, max_num + 1):
            weight = int(np.power(max(1, freq.get(i, 0)), power) * 10)
    ...
    def entropy_predict(self, history: List[Dict], lottery_rules: Dict, boost_z4: bool = False) -> Dict:
        """
        ...
        3. boost_z4: 強制提高 Z4 (31-38 號) 的權重，應對極端相位切換。
        """
        pick_count = lottery_rules.get('pickCount', 6)
        ...
        top_candidates = [n for n, c in freq.most_common(30)] # 擴大候選集
        ...
        for _ in range(pick_count):
            ...
            for n in range(1, max_num + 1):
                ...
                # ✨ Phase Fail-Analysis: boost_z4 邏輯
                if boost_z4 and n >= 31: # Z4 區間 (威力彩)
                    weighted_diff *= 0.1 # 極大提升權重 (越低越好)
        ...
        return {
            ...
            'confidence': 0.8 if boost_z4 else 0.7,
            'method': 'entropy_driven' + ('_z4_boosted' if boost_z4 else '')
        }
    ...
    def ensemble_predict(
        ...
        """
        # Phase 2: Check for MAB override
        if getattr(self, 'mab_predictor', None):
            try:
                # 偵測當前環境機制 (Order/Chaos)
                regime = 'GLOBAL'
                if getattr(self, 'regime_detector', None):
                    regime_info = self.regime_detector.detect_regime(history)
                    regime = regime_info.get('regime', 'GLOBAL')
                
                # Use expected weights for stable prediction, or sampled for exploration
                # Here we use sampled weights (default) to allow exploration with regime context
                return self.mab_predictor.predict(history, lottery_rules, regime=regime)
            except Exception as e:
                logger.error(f"MAB prediction failed, falling back to static ensemble: {e}")

        # Static Ensemble Logic (Fallback or Default)
        pick_count = lottery_rules.get('pickCount', 6)
        data_size = len(history)
    ...
    def update_mab(self, history: List[Dict], strategy_predictions: Dict[str, List[int]], 
                   actual_numbers: List[int], lottery_type: str = 'POWER_LOTTO'):
        """
        更新 MAB 權重 (環境感知)
        ✨ Phase Stability Audit: 加入 lottery_type 支持動態衰減
        """
        if not getattr(self, 'mab_predictor', None):
            return
        ...
        self.mab_predictor.update_with_result(strategy_predictions, actual_numbers, 
                                             regime=regime, lottery_type=lottery_type)
    ...
    def fourier_main_predict(self, history, lottery_rules):
        """
        主號碼傅立葉週期分析 (Multi-Scale Fourier)
        """
        try:
            ...
            # 使用 64, 128, 256 三個尺度
            scores = fourier_predictor.predict_main_numbers(history, max_num, window_sizes=[64, 128, 256])
            
            pick_count = lottery_rules.get('pickCount', 6)
            sorted_nums = sorted(scores.items(), key=lambda x: x[1], reverse=True)
            
            predicted = sorted([n for n, s in sorted_nums[:pick_count]])
            confidence = float(np.mean([s for n, s in sorted_nums[:pick_count]]))
            
            # 特別號由專用傅立葉預測
            # predict_special_number 在本文件中已定義，直接調用
            special = predict_special_number(history, lottery_rules, predicted, strategy_name='fourier')
            
            return {
                'numbers': predicted,
                'special': special,
                'confidence': confidence,
                'method': '主號傅立葉週期 (Main Fourier)'
            }
        except Exception as e:
            logger.error(f"主號傅立葉預測失敗: {e}")
            return self.statistical_predict(history, lottery_rules)

    def lag_reversion_predict(self, history: List[Dict], lottery_rules: Dict, boost_cold: bool = False) -> Dict:
        """
        滯後回歸與重複號策略 (Failure Analysis 115000010 產出)
        專門捕捉連開號碼與冷號回補
        """
        try:
            if not hasattr(self, 'lag_predictor') or self.lag_predictor is None:
                from .lag_reversion import LagReversionPredictor
                self.lag_predictor = LagReversionPredictor(lottery_rules)
            
            # 更新規則（防止彩種切換）
            self.lag_predictor.min_num = lottery_rules.get('minNumber', 1)
            self.lag_predictor.max_num = lottery_rules.get('maxNumber', 38)
            
            res = self.lag_predictor.predict(history, lottery_rules) # Placeholder for numbers, but we'll refine with scores
            res['method'] = '滯後回歸與重複號 (Catch Repeats)'
            res['special'] = predict_special_number(history, lottery_rules, res['numbers'], strategy_name='lag')
            
            # ✨ 新增：使用 ZoneClusterRefiner 進行微調
            if hasattr(self, 'zone_refiner') and self.zone_refiner:
                self.zone_refiner.max_num = lottery_rules.get('maxNumber', 38)
                # Pass boost_cold down to scores
                scores = self.lag_predictor.calculate_scores(history, boost_cold=boost_cold)
                refined_scores = self.zone_refiner.refine(history, scores)
                
                # Select top numbers based on refined scores
                predicted_numbers_refined = sorted(refined_scores.keys(), 
                                                   key=lambda n: refined_scores[n],
                                                   reverse=True)[:lottery_rules.get('pickCount', 6)]
                res['numbers'] = predicted_numbers_refined
            else:
                # If no refiner, still need to use boost_cold scores
                scores = self.lag_predictor.calculate_scores(history, boost_cold=boost_cold)
                top_nums = sorted(scores.items(), key=lambda x: x[1], reverse=True)[:lottery_rules.get('pickCount', 6)]
                res['numbers'] = sorted([n for n, s in top_nums])
            
            return res
        except Exception as e:
            logger.error(f"滯後回歸預測失敗: {str(e)}", exc_info=True)
            return self.frequency_predict(history, lottery_rules)
    ...
    def anomaly_detection_predict(self, history: List[Dict], lottery_rules: Dict) -> Dict:
        ...
        # 訓練並預測
        res = predictor.predict(history, pick_count=pick_count)
        predicted_numbers = res.get('numbers', []) if isinstance(res, dict) else res
    ...
    # ========================================================================
    # 🔥 P2優化: 號碼連莊/回補增強 (2026-01-13)
    # ========================================================================

    def repeat_booster_predict(self, history: List[Dict], lottery_rules: Dict) -> Dict:
        """
        連莊與回補增強策略 (Repeat Booster)
        
        核心原理：
        1. 統計 1-period repeat (連莊) 和 2-period rebound (隔期回補)
        2. 根據歷史連莊機率動態加權
        3. 針對 Big Lotto 最近頻繁出現的雙連莊 (01, 14) 進行模式強化
        """
        pick_count = lottery_rules.get('pickCount', 6)
        min_num = lottery_rules.get('minNumber', 1)
        max_num = lottery_rules.get('maxNumber', 49)
        ...
        last_2 = set(history[1]['numbers']) if len(history) > 1 else set()
        
        # 2. 計算歷史連莊率 (用於信心度)
        total_repeats = 0
        for i in range(min(100, len(history) - 1)):
            target = set(history[i]['numbers'])
            prev = set(history[i+1]['numbers'])
            total_repeats += len(target & prev)
        avg_repeat_count = total_repeats / min(100, len(history) - 1) if len(history) > 1 else 0
        ...
        if len(predicted) < pick_count:
            top_freq = [n for n, _ in freq.most_common(20) if n not in predicted]
            predicted.extend(top_freq[:pick_count - len(predicted)])

        confidence = 0.55 + (avg_repeat_count / pick_count) * 0.2
        
        return {
            'numbers': sorted(predicted[:pick_count]),
            'confidence': float(confidence),
            'method': '連莊/回補增強 (Repeat Booster)',
            'booster_info': {
                'avg_repeat_rate': avg_repeat_count,
                'last_draw_hit': list(last_1),
            }
        }
    ...
    def adaptive_weight_predict(self, history: List[Dict], lottery_rules: Dict) -> Dict:
        ...
        # 檢測歷史順序 Check History Order
        # We need RECENT history for backtesting.
        # If ASC (history[-1] is latest/newest), we want history[-30:]
        # If DESC (history[0] is latest/newest), we want history[0:30]
        ...
        for i in range(backtest_window):
            if is_asc:
                # ASC: Old -> New. Latest is at [-1].
                # We want the LAST 30 draws as targets.
                # slice: target is history[-(i+1)] (Draw T)
                # training is history[... : -(i+1)] (Draws before T)
                # But we need exactly 100 past draws for training?
                # history[-(i+1)-100 : -(i+1)]
                target_idx = -(i + 1)
                target = history[target_idx]
                start = target_idx - 100
                end = target_idx
                # slice behaves weirdly with negative indices sometimes if start < -len
                # convert to positive indices for safety
                pos_target = len(history) + target_idx
                test_history = history[pos_target - 100 : pos_target]
            else:
                # DESC: New -> Old. Latest is at [0].
                # We want the FIRST 30 draws as targets (Recent).
                # target is history[i]
                # training is history[i+1 : i+101] (Older than target)
                target = history[i]
    ...
    def anti_consensus_predict(self, history: List[Dict], lottery_rules: Dict) -> Dict:
        """反共識策略橋接方法"""
        from .anti_consensus_predictor import anti_consensus_predict
        return anti_consensus_predict(history, lottery_rules)

    def cluster_pivot_predict(self, history: List[Dict], lottery_rules: Dict) -> Dict:
        """聚類軸點策略橋接方法 (使用 MultiBetOptimizer 核心邏輯)"""
        from .multi_bet_optimizer import MultiBetOptimizer
        optimizer = MultiBetOptimizer()
        # 模擬為單注預測，取第一注
        res = optimizer.generate_diversified_bets(history, lottery_rules, num_bets=1, meta_config={'method': 'cluster_pivot'})
        if res['bets']:
            bet = res['bets'][0]
            return {
                'numbers': bet['numbers'],
                ...
                'confidence': 0.72,
                'method': '聚類軸點 (Cluster Pivot)'
            }
        return self.frequency_predict(history, lottery_rules)

    # ========================================================================
    # 🕸️ GNN: Graph Neural Network (Phase 46)
    # ========================================================================

    def gnn_predict(self, history: List[Dict], lottery_rules: Dict) -> Dict:
        """
        GNN 圖神經網路預測策略
        捕捉號碼間的「關聯性」與「空間結構」
        """
        try:
            from .gnn_predictor import gnn_predict
    ...
    def meta_stacking_predict(self, history: List[Dict], lottery_rules: Dict) -> Dict:
        """
        🚀 Phase 54: Deep Stacking Fusion (MLP-based)
        """
        max_num = lottery_rules.get('maxNumber', 38)
        regime_info = self.regime_detector.detect_regime(history)
        regime = regime_info.get('regime', 'GLOBAL')
        ...
        # 收集所有子模型分數
        strategy_scores = {}
        for s_name in self.meta_stacker.strategy_names:
            try:
                # 映射特例
                method_name = f"{s_name if 'fourier' not in s_name else 'fourier_main'}_predict"
                res = getattr(self, method_name)(history, lottery_rules)
                score_vec = {n: 0.0 for n in range(1, max_num + 1)}
        ...
        # 收集結構性特徵 (Phase 55: Enhanced Features)
        structural_features = {
            'entropy': {n: 0.0 for n in range(1, max_num + 1)},
            'volatility': {n: 0.0 for n in range(1, max_num + 1)}
        }
        ...
        current_entropy = self.analyzer.calculate_entropy(flattened_recent, max_num)
        
        freq_info = self.analyzer.get_draw_stats(history[-50:]) # 獲取近期統計
        
        for n in range(1, max_num + 1):
            structural_features['entropy'][n] = float(current_entropy)
            # 簡化波動率：基於號碼在 50 期內的頻率偏差
            n_freq = len([d for d in history[-50:] if n in d.get('numbers', [])])
            structural_features['volatility'][n] = abs(n_freq - (50 * 6 / max_num)) / 10.0

        # 進行深度融合
        refined_scores = self.meta_stacker.predict_refined_scores(strategy_scores, structural_features, regime, max_num)
        
        # 排序取前 N
        pick_count = lottery_rules.get('pickCount', 6)
        sorted_nums = sorted(refined_scores.items(), key=lambda x: x[1], reverse=True)
        top_numbers = [n for n, s in sorted_nums[:pick_count]]
        
        return {
            'numbers': sorted(top_numbers),
            'confidence': float(np.mean([s for n, s in sorted_nums[:pick_count]]) if sorted_nums else 0.5),
            'all_scores': refined_scores,
            'method': 'Meta_Stacking_2.0'
        }

    def diffusion_predict(self, history: List[Dict], lottery_rules: Dict) -> Dict:
        """
        🚀 Phase 56: Diffusion-based Generative Prediction
        """
        try:
            # Generate 1 sample (ticket)
            generated_bets = self.diffusion_gen.sample(num_samples=1)
            if not generated_bets:
                return self.frequency_predict(history, lottery_rules)
            
            final_numbers = generated_bets[0]
            return {
                'numbers': sorted(final_numbers),
                'confidence': 0.85, # Generative models often have high implicit confidence
                'method': 'Diffusion_Generative_DDPM'
            }
        except Exception as e:
            logger.error(f"Diffusion 預測失敗: {e}")
            return self.statistical_predict(history, lottery_rules)

    def vae_predict(self, history: List[Dict], lottery_rules: Dict, **kwargs) -> Dict:
        """
        🚀 Phase Post-115000011: VAE Latent Distribution Prediction
        利用變分自編碼器建模號碼空間，捕捉潛在規律。
        """
        if self.vae_model is None:
            return self.statistical_predict(history, lottery_rules)
            
        try:
            res = self.vae_model.predict(history, lottery_rules, **kwargs)
            return self.auto_identify_stability(lottery_rules.get('name', ''), 'VAE_Single', res)
        except Exception as e:
            logger.error(f"VAE 預測失敗: {e}")
            return self.statistical_predict(history, lottery_rules)
