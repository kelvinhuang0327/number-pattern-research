...
def test_p541b_r2_has_fresh_main_record_count(p541b_r2):
    assert len(p541b_r2["method_classification_records"]) == _module().SOURCE_MANIFEST_COUNT
...
def test_input_matches_pinned_identity(input_provenance):
    ...
    assert input_provenance["detector_version"] == "p541b-r2-detector-v10-fresh-main-r1"
    assert input_provenance["source_manifest_sha256"] == mod.SOURCE_MANIFEST_SHA256
    assert input_provenance["record_count"] == mod.SOURCE_MANIFEST_COUNT
...
def test_load_verified_input_reads_pinned_path_exactly_once(
    ...
    assert len(loaded["method_classification_records"]) == mod.SOURCE_MANIFEST_COUNT
...
def test_bucket_totals_sum_to_fresh_main_record_count(p541b_r2):
    mod = _module()
    decisions = mod.classify_all(p541b_r2)
    summary = mod.summarize(decisions)
    total = sum(summary[b] for b in mod.ALL_BUCKETS)
    assert total == mod.SOURCE_MANIFEST_COUNT
    assert summary["total_reviewed_from_p541b_r2"] == mod.SOURCE_MANIFEST_COUNT
    assert [summary[b] for b in mod.ALL_BUCKETS] == [0, 13, 0, 509, 71]
...
def test_build_performs_no_per_record_source_filesystem_io(
    ...
    assert len(artifact["reviewed_method_decisions"]) == mod.SOURCE_MANIFEST_COUNT
...
def test_invariant_low_risk_does_not_erase_upstream_readiness_contract(p541b_r2):
    ...
    assert len(low_confirmed_adapter) == 13
...
def test_shortlist_exact_membership_and_contract(p541b_r2):
    ...
    assert tuple(d["method_id"] for d in shortlist) == (
        mod.EXPECTED_SUCCESSOR_SHORTLIST_PATHS
    )
...
def test_build_artifact_has_all_required_sections(p541b_r2, input_provenance):
    ...
    required_sections = [
        ...
        "input_provenance", "fresh_main_chain_repair", "provenance_and_limits",
        "disclaimer", "supersedes",
    ]
...
def test_selector_version_is_exact_and_fails_closed(p541b_r2, input_provenance):
    mod = _module()
    artifact = mod.build_artifact(p541b_r2, input_provenance, generated_at="TEST")
    assert artifact["selector_version"] == "p541c-r2-selector-v5-fresh-main-r1"
...
def test_build_artifact_is_json_serializable(p541b_r2, input_provenance):
    ...
    assert reloaded["summary"]["total_reviewed_from_p541b_r2"] == mod.SOURCE_MANIFEST_COUNT
...
def test_render_markdown_contains_key_sections(p541b_r2, input_provenance):
    ...
    assert "## Fresh-Main Chain Repair" in md
    assert "## Bucket Definitions" in md
    assert "## Shortlist" in md
    assert "> selector_version: p541c-r2-selector-v5-fresh-main-r1" in md
...
def test_committed_json_and_markdown_agree():
    ...
    assert committed_json["selector_version"] == "p541c-r2-selector-v5-fresh-main-r1"
