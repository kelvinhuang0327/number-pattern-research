...
## 📊 驗證報告索引 (2026-01-05 更新)

> **🎯 核心推薦**: 今彩539 3注 (37.14% 中獎率) | 威力彩 6注 (22.11% 中獎率) | 大樐透 8注 (15.52% 中獎率)

**完整驗證報告**: [BACKTEST_REPORTS_INDEX.md](../docs/BACKTEST_REPORTS_INDEX.md)

| 樂透 | 最優策略 | 中獎率 | 驗證期數 | 詳細報告 |
|-----|--------|--------|----------|---------|
| 🔥 **今彩539** | 3注 | **37.14%** | 315期 | [DAILY539_MULTI_BET_BACKTEST_REPORT.md](../docs/DAILY539_MULTI_BET_BACKTEST_REPORT.md) |
| ⭐ **威力彩** | 6注 | **22.11%** | 95期 | [POWER_LOTTO_MULTI_BET_BACKTEST_REPORT.md](../docs/POWER_LOTTO_MULTI_BET_BACKTEST_REPORT.md) |
| 📘 **大樐透** | 8注 | **15.52%** | 116期 | [BIG_LOTTO_MULTI_BET_BACKTEST_REPORT.md](../docs/BIG_LOTTO_MULTI_BET_BACKTEST_REPORT.md) |
...
|------|--------|-----------|---------|------|
| 單注 | 4.31% | 23.2期 | 116期 | ✅ 已驗證 |
| 2 注 | **8.62%** | **11.6期** | **116期** | **✅ 已驗證** |
| 4 注 | ~8.5% | ~11.8期 | 116期 | ✅ 已驗證 |
| 6 注 (標準) | 13.79% | 7.3期 | 116期 | ✅ 已驗證 |
| 6 注 (P2優化) | **9.32%** | **10.7期** | ~103期 | 🔥 **Smart Wobble增強** |
| 8 注 | **15.52%** | **6.4期** | **116期** | **✅ 已驗證** |

> ⚠️ **重要提醒**: 大樂透難度最高，最高中獎率僅 15.52%，**無法達成 33% 目標**。建議優先選擇今彩539或威力彩。
> 📊 **詳細驗證報告**: [BIG_LOTTO_MULTI_BET_BACKTEST_REPORT.md](../docs/BIG_LOTTO_MULTI_BET_BACKTEST_REPORT.md)

**🔥 P2優化 (2026-01-04)**: 使用 Smart Wobble 策略，6注中獎率從 6.78% 提升至 **9.32%** (+2.54%)
- ✅ 驗證報告: [Smart Wobble 優化詳情](../docs/BIG_LOTTO_MULTI_BET_BACKTEST_REPORT.md#4-smart-wobble-策略優化)

**最佳 2 注組合**: `zone_balance(500)` + `zone_balance(200)` 或 `bayesian(300)` 或 `trend(300)`
- ✅ 驗證報告: [2注策略詳細驗證](../docs/BIG_LOTTO_MULTI_BET_BACKTEST_REPORT.md#21-2注策略-862)

| 注數 | 中獎率 | 每N期中1次 | 驗證期數 | 狀態 |
|------|--------|-----------|---------|---------|
| 2 注 | 8.42% | 11.9 期 | 95期 | 基礎版 |
| **4 注 (ClusterPivot)** | **~15%** | **~6.8 期** | **102期** | **✅ 獨立驗證 (2026-01-05)** |
| 6 注 | **22.11%** | **4.5 期** | **95期** | **✅ 獨立驗證** |
| 8 注 | **31.58%** | **3.2 期** | **95期** | **✅ 獨立驗證** |

> ⚠️ **數據更正 (2026-01-04)**: 原 P5/P7 聲稱的 40-44% 中獎率為「所有獎項」合計且存在數據洩漏，經「無洩漏」獨立驗證 **Match-3+** 實際為 **14.74%-31.58%**。
> 原因：1. 排除號碼使用了未來數據 (Leakage)；2. 特別號偏差表使用了全局統計。特別號命中率修正為 **~50%**。
> 📊 **詳細驗證報告**: [POWER_LOTTO_MULTI_BET_BACKTEST_REPORT.md](../docs/POWER_LOTTO_MULTI_BET_BACKTEST_REPORT.md)

#### ClusterPivot 核心參數 (已驗證配置)
-   **威力彩 (4注)**: `method='cluster_pivot'`, `anchor_count=2`, `resilience=True` -> **~15%** (特別號 ~50%)
    - ✅ 驗證來源: [POWER_LOTTO_cluster_pivot_4bets_2025.json](data/backtest_results/POWER_LOTTO_cluster_pivot_4bets_2025.json)
    - ✅ 獨立驗證 (2026-01-05): 3次運行平均 **15.69%** (13.73%, 18.63%, 14.71%)
    - ✅ 驗證期數: 102期 (2025年威力彩全年)
    - ✅ 防洩漏檢查: 滾動式回測，使用該期後的全部歷史
-   **特別號優化**: `_get_sum_biased_specials` 跨區關聯有效，從隨機 12.5% 提升至 **~50%**
    - ✅ 驗證來源: [POWER_LOTTO_MULTI_BET_BACKTEST_REPORT.md - 第4.3節](../docs/POWER_LOTTO_MULTI_BET_BACKTEST_REPORT.md#43-特別號優化驗證)
    - ✅ 改善幅度: +37.5% (12.5% → 50%)
    - ✅ 實驗基礎: 95期滾動式驗證 (無洩漏)
...
|------|--------|-----------|---------|---------|---------|
| 單注 | 11.75% | 8.5 期 | $50 | 1.26x | ✅ 已驗證 |
| 2 注 | 27.62% | 3.6 期 | $100 | 2.97x | ✅ 已驗證 |
| **3 注** | **37.14%** | **2.7 期** | **$150** | **3.99x** | **✅ 已驗證** 🔥🔥 |
| 4 注 | **~42%** | **~2.4 期** | **$200** | **~4.5x** | **部分驗證** |

**🔥🔥 推薦: 3注覆蓋策略 (37.14% 中獎率) — 達成33%目標！**

> ⚠️ **重要發現**: 2注組合無法達成33%目標 (最高約28%)，需要3注才能達標
> 📊 **詳細驗證報告**: [DAILY539_MULTI_BET_BACKTEST_REPORT.md](../docs/DAILY539_MULTI_BET_BACKTEST_REPORT.md)

**回測驗證結果 (2025年315期，35種組合測試)**:
- ✅ 第1注: `sum_range` 方法 (窗口300期) — 和值範圍分析 | 中獎率 15.34%
- ✅ 第2注: `bayesian` 方法 (窗口300期) — 貝葉斯統計分析 | 中獎率 14.38%
- ✅ 第3注: `zone_opt` 方法 (窗口200期) — 區間優化分析 | 中獎率 14.06%
- ✅ 任一注中2個號碼以上即為成功 | 組合中獎率 37.14%
- 📈 完整驗證: [3注策略詳細驗證](../docs/DAILY539_MULTI_BET_BACKTEST_REPORT.md#22-3注策略-3714-🔥🔥-推薦)
...
- **2026-01-04**: ⚠️ **P5/P6/P7 數據更正 - 誠信審計與洩漏修復**
  - **原聲稱**: P5 威力彩 4注 40.2%, P7 威力彩 4注 44.07%, 特別號 53%
  - **因應 Claude 警示修訂 (2026-01-05)**:
    - 威力彩 4注 ClusterPivot: **~15%** (Match-3+ 標竿)
    - 特別號命中率: **~50%** (跨區關聯優化)
  - **洩漏診斷**:
    - 殺號模型 (`NegativeSelector`) 曾錯誤使用全量數據而非滾動歷史。
    - 特別號偏差表 (`Sum-Bias`) 包含測試期間統計，現改用滾動上下文。
    - 判定標準從「全獎項」收斂至「Match-3+」。
