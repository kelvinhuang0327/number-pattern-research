...
class TransformerZDPPredictor:
    ...
    def predict(self, history, rules):
        ...
        return {'numbers': final_numbers, 'method': 'AI-Lab Transformer + ZDP'}
...
def main():
    ...
    ai_predictor = TransformerZDPPredictor(model_path)
    hpsb_v2 = HPSBOptimizer(UnifiedPredictionEngine())
    
    print("=" * 65)
    print("🔬 AI-Lab Transformer + ZDP Comparative Benchmark")
    print("=" * 65)
    
    strategies = [
        ("DMS (SOTA Baseline)", hpsb_v2.predict_hpsb_v2),
        ("Transformer (Raw)", None), # Placeholder for old logic if needed
        ("Transformer + ZDP", ai_predictor.predict)
    ]
    for name, func in strategies:
        if name == "Transformer (Raw)":
            from scripts.benchmark_ai import TransformerPredictor
            old_ai = TransformerPredictor(model_path)
            func = old_ai.predict
