...
class MissingIssueDetector:
    ...
    def scan(self, lottery_type: str,
        ...
        def _draw_sort_key(x):
            try:
                return (1, int(x))
            except (ValueError, TypeError):
                return (0, x)   # non-numeric draws sort before numeric ones
    ...
    def _detect_internal_gaps(self, draws: List[str]) -> List[Dict]:
        ...
        def _split(draw: str):
            ...
            # Returns None for non-standard formats (e.g. '009-01').
        ...
        for i in range(1, len(draws)):
            prev = _split(draws[i - 1])
            curr = _split(draws[i])
            if prev is None or curr is None:
                continue      # skip non-standard draw numbers
            prev_year, prev_seq = prev
            curr_year, curr_seq = curr
